...
2. Fork [athena from the CryptAthlos repository](https://github.com/cryptathlos/athena)
...
**1. in file src/CryptAthlosConfig.h** - `CRYPTATHLOS_NAME` constant

Example: 
```
const char CRYPTATHLOS_NAME[] = "olympuscoin";
```
...
**1. Total money supply** (src/CryptAthlosConfig.h)

Total amount of coins to be emitted. Most of CryptAthlos based coins use `(uint64_t)(-1)` (equals to 18446744073709551616). You can define number explicitly (for example `UINT64_C(858986905600000000)`).
...
**2. Emission curve** (src/CryptAthlosConfig.h)

Be default CryptAthlos provides emission formula with slight decrease of block reward with each block. This is different from Bitcoin where block reward halves every 4 years.
...
**3. Difficulty target** (src/CryptAthlosConfig.h)
...
In case you are not satisfied with CryptAthlos default implementation of block reward logic you can also change it. The implementation is in `src/CryptAthlosCore/Currency.cpp`:
```
...
- big block penalty calculation: this is the way CryptAthlos protects the block chain from transaction flooding attacks and preserves opportunities for organic network growth at the same time.
...
**1. Default ports for P2P and RPC networking** (src/CryptAthlosConfig.h)
...
```
const static boost::uuids::uuid CRYPTATHLOS_NETWORK = { { 0xA1, 0x1A, 0xA1, 0x1A, 0xA1, 0x0A, 0xA1, 0x0A, 0xA0, 0x1A, 0xA0, 0x1A, 0xA0, 0x1A, 0xA1, 0x1A } };
```


**3. Seed nodes** (src/CryptAthlosConfig.h)
...
**1. Minimum transaction fee** (src/CryptAthlosConfig.h)
...
**2. Penalty free block size** (src/CryptAthlosConfig.h)

CryptAthlos protects chain from tx flooding by reducing block reward for blocks larger than the median block size. However, this rule applies for blocks larger than `CRYPTATHLOS_BLOCK_GRANTED_FULL_REWARD_ZONE` bytes.

Example:
```
const size_t CRYPTATHLOS_BLOCK_GRANTED_FULL_REWARD_ZONE = 20000;
```
...
Example:
```
const uint64_t CRYPTATHLOS_PUBLIC_ADDRESS_BASE58_PREFIX = 0xe9; // addresses start with "f"
```


### Sixth step. Genesis block

**1. Build the binaries with blank genesis tx hex** (src/CryptAthlosConfig.h)
...
**3. Copy the printed transaction hash** (src/CryptAthlosConfig.h)

Copy the tx hash that has been printed by the daemon to `GENESIS_COINBASE_TX_HEX` in `src/CryptAthlosConfig.h`
