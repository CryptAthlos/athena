...
bool one_block::verify_1(CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
    DEFINE_TESTS_ERROR_CONTEXT("one_block::verify_1");

    alice = boost::get<CryptAthlos::AccountBase>(events[1]);

    // check balances
    //std::vector<const CryptAthlos::Block*> chain;
    //map_hash2tx_t mtx;
    //CHECK_TEST_CONDITION(find_block_chain(events, chain, mtx, get_block_hash(boost::get<CryptAthlos::Block>(events[1]))));
    //CHECK_TEST_CONDITION(get_block_reward(0) == get_balance(alice, events, chain, mtx));

    // check height
    std::list<CryptAthlos::Block> blocks;
    ...
    CHECK_TEST_CONDITION(blocks.back() == boost::get<CryptAthlos::Block>(events[0]));
...
bool gen_simple_chain_001::generate(std::vector<test_event_entry> &events)
{
    ...
    map_hash2tx_t mtx;
    /*bool r = */find_block_chain(events, chain, mtx, get_block_hash(boost::get<CryptAthlos::Block>(events[3])));
...
bool gen_simple_chain_001::verify_callback_1(CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
  return true;
}

bool gen_simple_chain_001::verify_callback_2(CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
  return true;
}
