...
bool GetRandomOutputs::request(CryptAthlos::core& c, uint64_t amount, size_t mixin, CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp) {
  CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request req;

  req.amounts.push_back(amount);
  req.outs_count = mixin;

  resp = boost::value_initialized<CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response>();
...
bool GetRandomOutputs::checkHalfUnlocked(CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
...
bool GetRandomOutputs::checkFullyUnlocked(CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
