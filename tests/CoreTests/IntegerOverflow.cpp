...
namespace
{
  ...
  void append_TransactionSourceEntry(std::vector<CryptAthlos::TransactionSourceEntry>& sources, const Transaction& tx, size_t out_idx)
  {
    CryptAthlos::TransactionSourceEntry se;
    se.amount = tx.outputs[out_idx].amount;
    se.outputs.push_back(std::make_pair(0, boost::get<CryptAthlos::KeyOutput>(tx.outputs[out_idx].target).key));
    se.realOutput = 0;
...
bool gen_uint_overflow_base::check_tx_verification_context(const CryptAthlos::tx_verification_context& tvc, bool tx_added, size_t event_idx, const CryptAthlos::Transaction& /*tx*/)
{
  return m_last_valid_block_event_idx < event_idx ? !tx_added && tvc.m_verifivation_failed : tx_added && !tvc.m_verifivation_failed;
}

bool gen_uint_overflow_base::check_block_verification_context(const CryptAthlos::block_verification_context& bvc, size_t event_idx, const CryptAthlos::Block& /*block*/)
{
...
bool gen_uint_overflow_base::mark_last_valid_block(CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
...
bool gen_uint_overflow_1::generate(std::vector<test_event_entry>& events) const
{
  ...
  std::list<CryptAthlos::Transaction> txs_1;
