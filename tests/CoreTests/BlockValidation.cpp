...
namespace {
  bool lift_up_difficulty(const CryptAthlos::Currency& currency, std::vector<test_event_entry>& events,
                          std::vector<uint64_t>& timestamps,
                          std::vector<CryptAthlos::difficulty_type>& cummulative_difficulties, test_generator& generator,
                          size_t new_block_count, const CryptAthlos::Block blk_last,
                          const CryptAthlos::AccountBase& miner_account, uint8_t block_major_version) {
    CryptAthlos::difficulty_type commulative_diffic = cummulative_difficulties.empty() ? 0 : cummulative_difficulties.back();
    CryptAthlos::Block blk_prev = blk_last;
    for (size_t i = 0; i < new_block_count; ++i) {
      CryptAthlos::Block blk_next;
      CryptAthlos::difficulty_type diffic = currency.nextDifficulty(timestamps, cummulative_difficulties);
  ...
  void clearTransaction(CryptAthlos::Transaction& tx) {
    tx.version = 0;
    tx.unlockTime = 0;
    tx.inputs.clear();
    tx.outputs.clear();
...
bool gen_block_invalid_prev_id::check_block_verification_context(const CryptAthlos::block_verification_context& bvc, size_t event_idx, const CryptAthlos::Block& /*blk*/)
{
  if (1 == event_idx)
...
gen_block_invalid_binary_format::gen_block_invalid_binary_format() : 
  ...
  CryptAthlos::CurrencyBuilder currencyBuilder(m_logger);
...
bool gen_block_invalid_binary_format::check_block_verification_context(const CryptAthlos::block_verification_context& bvc,
                                                                       size_t event_idx, const CryptAthlos::Block& blk)
{
...
bool gen_block_invalid_binary_format::corrupt_blocks_boundary(CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  m_corrupt_blocks_begin_idx = ev_index + 1;
  return true;
}

bool gen_block_invalid_binary_format::check_all_blocks_purged(CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
