...
TransactionBuilder::TransactionBuilder(const CryptAthlos::Currency& currency, uint64_t unlockTime)
  : m_currency(currency), m_version(CryptAthlos::CURRENT_TRANSACTION_VERSION), m_unlockTime(unlockTime), m_txKey(generateKeyPair()) {}
...
TransactionBuilder& TransactionBuilder::setTxKeys(const CryptAthlos::KeyPair& txKeys) {
  m_txKey = txKeys;
  return *this;
}

TransactionBuilder& TransactionBuilder::setInput(const std::vector<CryptAthlos::TransactionSourceEntry>& sources, const CryptAthlos::AccountKeys& senderKeys) {
  m_sources = sources;
...
TransactionBuilder& TransactionBuilder::setOutput(const std::vector<CryptAthlos::TransactionDestinationEntry>& destinations) {
  m_destinations = destinations;
  return *this;
}

TransactionBuilder& TransactionBuilder::addOutput(const CryptAthlos::TransactionDestinationEntry& dest) {
...
Transaction TransactionBuilder::build() const {
  ...
  std::vector<CryptAthlos::KeyPair> contexts;
...
void TransactionBuilder::fillInputs(Transaction& tx, std::vector<CryptAthlos::KeyPair>& contexts) const {
...
void TransactionBuilder::signSources(const Crypto::Hash& prefixHash, const std::vector<CryptAthlos::KeyPair>& contexts, Transaction& tx) const {
