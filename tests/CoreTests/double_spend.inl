...
template<class concrete_test>
bool gen_double_spend_base<concrete_test>::check_tx_verification_context(const CryptAthlos::tx_verification_context& tvc, bool tx_added, size_t event_idx, const CryptAthlos::Transaction& /*tx*/)
{
...
template<class concrete_test>
bool gen_double_spend_base<concrete_test>::check_block_verification_context(const CryptAthlos::block_verification_context& bvc, size_t event_idx, const CryptAthlos::Block& /*block*/)
{
...
template<class concrete_test>
bool gen_double_spend_base<concrete_test>::mark_last_valid_block(CryptAthlos::core& c, size_t /*ev_index*/, const std::vector<test_event_entry>& /*events*/)
{
  std::list<CryptAthlos::Block> block_list;
...
template<class concrete_test>
bool gen_double_spend_base<concrete_test>::mark_invalid_tx(CryptAthlos::core& /*c*/, size_t ev_index, const std::vector<test_event_entry>& /*events*/)
{
...
template<class concrete_test>
bool gen_double_spend_base<concrete_test>::mark_invalid_block(CryptAthlos::core& /*c*/, size_t ev_index, const std::vector<test_event_entry>& /*events*/)
{
...
template<class concrete_test>
bool gen_double_spend_base<concrete_test>::check_double_spend(CryptAthlos::core& c, size_t /*ev_index*/, const std::vector<test_event_entry>& events)
{
  ...
  CryptAthlos::AccountBase bob_account = boost::get<CryptAthlos::AccountBase>(events[1]);
  CryptAthlos::AccountBase alice_account = boost::get<CryptAthlos::AccountBase>(events[2]);

  std::vector<CryptAthlos::Block> chain;
  map_hash2tx_t mtx;
  std::vector<CryptAthlos::Block> blocks(block_list.begin(), block_list.end());
...
bool gen_double_spend_in_tx<txs_keeped_by_block>::generate(std::vector<test_event_entry>& events) const
{
  ...
  CryptAthlos::TransactionSourceEntry se;
  se.amount = tx_0.outputs[0].amount;
  se.outputs.push_back(std::make_pair(0, boost::get<CryptAthlos::KeyOutput>(tx_0.outputs[0].target).key));
  se.realOutput = 0;
  se.realTransactionPublicKey = CryptAthlos::getTransactionPublicKeyFromExtra(tx_0.extra);
  se.realOutputIndexInTransaction = 0;
  sources.push_back(se);
  // Double spend!
  sources.push_back(se);

  CryptAthlos::TransactionDestinationEntry de;
...
bool gen_double_spend_in_the_same_block<txs_keeped_by_block>::generate(std::vector<test_event_entry>& events) const
{
  ...
  CryptAthlos::Transaction tx_1 = txs_1.front();
