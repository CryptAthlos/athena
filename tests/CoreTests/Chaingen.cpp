...
//#include "cryptathlos_boost_serialization.h"

using namespace std;
using namespace CryptAthlos;

struct output_index {
    const CryptAthlos::TransactionOutputTarget out;
    uint64_t amount;
    ...
    uint32_t idx;
    bool spent;
    const CryptAthlos::Block *p_blk;
    const CryptAthlos::Transaction *p_tx;

    output_index(const CryptAthlos::TransactionOutputTarget &_out, uint64_t _a, size_t _h, size_t tno, size_t ono, const CryptAthlos::Block *_pb, const CryptAthlos::Transaction *_pt)
...
bool init_output_indices(map_output_idx_t& outs, std::map<uint64_t, std::vector<size_t> >& outs_mine, const std::vector<CryptAthlos::Block>& blockchain, const map_hash2tx_t& mtx, const CryptAthlos::AccountBase& from) {
...
bool init_spent_output_indices(map_output_idx_t& outs, map_output_t& outs_mine, const std::vector<CryptAthlos::Block>& blockchain, const map_hash2tx_t& mtx, const CryptAthlos::AccountBase& from) {
...
bool fill_tx_sources(std::vector<TransactionSourceEntry>& sources, const std::vector<test_event_entry>& events,
                     const Block& blk_head, const CryptAthlos::AccountBase& from, uint64_t amount, size_t nmix)
{
    map_output_idx_t outs;
    map_output_t outs_mine;

    std::vector<CryptAthlos::Block> blockchain;
    map_hash2tx_t mtx;
    ...
    BOOST_REVERSE_FOREACH(const map_output_t::value_type o, outs_mine)
    {
        for (size_t i = 0; i < o.second.size() && !sources_found; ++i)
        {
            ...
            CryptAthlos::TransactionSourceEntry ts;
            ts.amount = oi.amount;
...
bool fill_tx_destination(TransactionDestinationEntry &de, const CryptAthlos::AccountBase &to, uint64_t amount) {
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, CryptAthlos::Transaction& tx, const Block& blk_head,
                         const CryptAthlos::AccountBase& from, const CryptAthlos::AccountBase& to, uint64_t amount,
...
const CryptAthlos::Currency& test_chain_unit_base::currency() const
{
  return m_currency;
}
...
bool test_chain_unit_base::verify(const std::string& cb_name, CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
