...
#include "CryptAthlosCore/Core.h"
#include "CryptAthlosCore/TransactionExtra.h"
...
struct serialized_object
{
  ...
  serialized_object(const CryptAthlos::BinaryArray& a_data)
    : data(a_data)
  {
  }

  CryptAthlos::BinaryArray data;
  //BEGIN_SERIALIZE_OBJECT()
  //  FIELD(data)
  //  END_SERIALIZE()
...
typedef serialized_object<CryptAthlos::Block> serialized_block;
typedef serialized_object<CryptAthlos::Transaction> serialized_transaction;
...
//VARIANT_TAG(binary_archive, CryptAthlos::account_base, 0xcc);
//VARIANT_TAG(binary_archive, serialized_block, 0xcd);
//VARIANT_TAG(binary_archive, serialized_transaction, 0xce);
//VARIANT_TAG(binary_archive, event_visitor_settings, 0xcf);

typedef boost::variant<CryptAthlos::Block, CryptAthlos::Transaction, CryptAthlos::AccountBase, callback_entry, serialized_block, serialized_transaction, event_visitor_settings> test_event_entry;
typedef std::unordered_map<Crypto::Hash, const CryptAthlos::Transaction*> map_hash2tx_t;

class test_chain_unit_base: boost::noncopyable
{
public:
  test_chain_unit_base() :
    m_currency(CryptAthlos::CurrencyBuilder(m_logger).currency()) {
  }

  typedef std::function<bool (CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry> &events)> verify_callback;
  typedef std::map<std::string, verify_callback> callbacks_map;

  const CryptAthlos::Currency& currency() const;
  void register_callback(const std::string& cb_name, verify_callback cb);
  bool verify(const std::string& cb_name, CryptAthlos::core& c, size_t ev_index, const std::vector<test_event_entry> &events);

protected:

  mutable Logging::ConsoleLogger m_logger;
  CryptAthlos::Currency m_currency;
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, CryptAthlos::Transaction& tx,
                         const CryptAthlos::Block& blk_head, const CryptAthlos::AccountBase& from, const CryptAthlos::AccountBase& to,
                         uint64_t amount, uint64_t fee, size_t nmix);
CryptAthlos::Transaction construct_tx_with_fee(Logging::ILogger& logger, std::vector<test_event_entry>& events, const CryptAthlos::Block& blk_head,
                                            const CryptAthlos::AccountBase& acc_from, const CryptAthlos::AccountBase& acc_to,
                                            uint64_t amount, uint64_t fee);

void get_confirmed_txs(const std::vector<CryptAthlos::Block>& blockchain, const map_hash2tx_t& mtx, map_hash2tx_t& confirmed_txs);
bool find_block_chain(const std::vector<test_event_entry>& events, std::vector<CryptAthlos::Block>& blockchain, map_hash2tx_t& mtx, const Crypto::Hash& head);
void fill_tx_sources_and_destinations(const std::vector<test_event_entry>& events, const CryptAthlos::Block& blk_head,
                                      const CryptAthlos::AccountBase& from, const CryptAthlos::AccountBase& to,
                                      uint64_t amount, uint64_t fee, size_t nmix,
                                      std::vector<CryptAthlos::TransactionSourceEntry>& sources,
                                      std::vector<CryptAthlos::TransactionDestinationEntry>& destinations);
uint64_t get_balance(const CryptAthlos::AccountBase& addr, const std::vector<CryptAthlos::Block>& blockchain, const map_hash2tx_t& mtx);

//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_tx_verification_context(const CryptAthlos::tx_verification_context& tvc, bool tx_added, size_t event_index, const CryptAthlos::Transaction& tx, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_tx_verification_context(const CryptAthlos::tx_verification_context& tvc, bool tx_added, size_t /*event_index*/, const CryptAthlos::Transaction& /*tx*/, t_test_class&, long)
{
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_tx_verification_context(const CryptAthlos::tx_verification_context& tvc, bool tx_added, size_t event_index, const CryptAthlos::Transaction& tx, t_test_class& validator)
{
  // SFINAE in action
...
//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_block_verification_context(const CryptAthlos::block_verification_context& bvc, size_t event_index, const CryptAthlos::Block& blk, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_block_verification_context(const CryptAthlos::block_verification_context& bvc, size_t /*event_index*/, const CryptAthlos::Block& /*blk*/, t_test_class&, long)
{
  // Default block verification context check
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_block_verification_context(const CryptAthlos::block_verification_context& bvc, size_t event_index, const CryptAthlos::Block& blk, t_test_class& validator)
{
  // SFINAE in action
...
public:
  push_core_event_visitor(CryptAthlos::core& c, const std::vector<test_event_entry>& events, t_test_class& validator)
    : m_c(c)
    , m_events(events)
  ...
  bool operator()(const CryptAthlos::Transaction& tx) const
  {
    log_event("CryptAthlos::Transaction");
  ...
  bool operator()(const CryptAthlos::Block& b) const
  {
    log_event("CryptAthlos::Block");

    CryptAthlos::block_verification_context bvc = boost::value_initialized<decltype(bvc)>();
  ...
  bool operator()(const CryptAthlos::AccountBase& ab) const
  {
    log_event("CryptAthlos::account_base");
    return true;
  }
  ...
  bool operator()(const serialized_block& sr_block) const
  {
    ...
    CryptAthlos::Block blk;
    if (!CryptAthlos::fromBinaryArray(blk, sr_block.data)) {
      blk = CryptAthlos::Block();
    }
  ...
  bool operator()(const serialized_transaction& sr_tx) const
  {
    ...
    if (!CryptAthlos::fromBinaryArray(tx, sr_tx.data)) {
      tx = CryptAthlos::Transaction();
    }
...
//--------------------------------------------------------------------------
template<class t_test_class>
inline bool replay_events_through_core(CryptAthlos::core& cr, const std::vector<test_event_entry>& events, t_test_class& validator)
{
  try {
    CHECK_AND_ASSERT_MES(typeid(CryptAthlos::Block) == events[0].type(), false, "First event must be genesis block creation");
    cr.set_genesis_block(boost::get<CryptAthlos::Block>(events[0]));
...
inline bool do_replay_events(std::vector<test_event_entry>& events, t_test_class& validator)
{
  ...
  CryptAthlos::CoreConfig::initOptions(desc);
  ...
  coreConfig.init(vm);
  CryptAthlos::MinerConfig emptyMinerConfig;
  CryptAthlos::cryptathlos_protocol_stub pr; //TODO: stub only for this kind of test, make real validation of relayed objects
  CryptAthlos::core c(validator.currency(), &pr, logger);
...
#define MAKE_NEXT_BLOCK_TX1(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, TX1)         \
  CryptAthlos::Block BLK_NAME;                                                         \
  {                                                                                   \
    std::list<CryptAthlos::Transaction> tx_list;                                       \
...
#define REWIND_BLOCKS_N(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, COUNT)           \
  CryptAthlos::Block BLK_NAME;                                                         \
  {                                                                                   \
    CryptAthlos::Block blk_last = PREV_BLOCK;                                          \
...
#define MAKE_TX_MIX(VEC_EVENTS, TX_NAME, FROM, TO, AMOUNT, NMIX, HEAD)                                   \
  CryptAthlos::Transaction TX_NAME;                                                                       \
...
#define MAKE_TX_LIST_START(VEC_EVENTS, SET_NAME, FROM, TO, AMOUNT, HEAD) \
    std::list<CryptAthlos::Transaction> SET_NAME; \
...
const uint64_t COIN = Pow10<CryptAthlos::parameters::CRYPTATHLOS_DISPLAY_DECIMAL_POINT>::value;
