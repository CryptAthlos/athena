...
public:
  ...
  SimpleTest(const CryptAthlos::Currency& currency, System::Dispatcher& system, const Tests::Common::BaseFunctionalTestsConfig& config) :
    BaseFunctionalTests(currency, system, config) {}

  class WaitForActualGrowObserver : public CryptAthlos::IWalletLegacyObserver {
  ...
  class WaitForActualDwindleObserver : public CryptAthlos::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForPendingGrowObserver : public CryptAthlos::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForConfirmationObserver : public CryptAthlos::IWalletLegacyObserver {
  ...
  class WaitForSendCompletedObserver : public CryptAthlos::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_Sent;
    std::error_code& m_error;
    CryptAthlos::TransactionId& m_transactionId;

  public:
    WaitForSendCompletedObserver(Tests::Common::Semaphore& Sent, CryptAthlos::TransactionId& transactionId, std::error_code& error) : m_Sent(Sent), m_transactionId(transactionId), m_error(error) { }
  ...
  class WaitForExternalTransactionObserver : public CryptAthlos::IWalletLegacyObserver {
  public:
    WaitForExternalTransactionObserver() { }
    std::promise<CryptAthlos::TransactionId> promise;
  ...
  class WaitForTransactionUpdated : public CryptAthlos::IWalletLegacyObserver {
  public:
    ...
    CryptAthlos::TransactionId expectindTxId;
  };
  ...
  bool perform1() {
    ...
    std::unique_ptr<CryptAthlos::INode> node1;
    std::unique_ptr<CryptAthlos::INode> node2;

    nodeDaemons.front()->makeINode(node1);
    nodeDaemons.front()->makeINode(node2);

    std::unique_ptr<CryptAthlos::IWalletLegacy> wallet1;
    std::unique_ptr<CryptAthlos::IWalletLegacy> wallet2;
  ...
  class WaitForBlockchainHeightChangeObserver : public CryptAthlos::INodeObserver {
  ...
  class CallbackHeightChangeObserver : public CryptAthlos::INodeObserver {
  ...
  bool perform2(size_t blocksCount = 10)
  {
    ...
    mineBlock();
    LOG_TRACE("STEP 2 PASSED");
    std::unique_ptr<CryptAthlos::INode> localNode;
    std::unique_ptr<CryptAthlos::INode> remoteNode;

    nodeDaemons.front()->makeINode(localNode);
    nodeDaemons.back()->makeINode(remoteNode);
    
    std::unique_ptr<CryptAthlos::IWalletLegacy> wallet;
  ...
  bool perform4() {
    ...
    std::unique_ptr<CryptAthlos::INode> hopNode;
    std::unique_ptr<CryptAthlos::INode> localNode;
    std::unique_ptr<CryptAthlos::INode> remoteNode;
  ...
  bool perform5() {
    ...
    std::unique_ptr<CryptAthlos::INode> node1;
    std::unique_ptr<CryptAthlos::INode> inprocNode;
...
void testMultiVersion(const CryptAthlos::Currency& currency, System::Dispatcher& d, const Tests::Common::BaseFunctionalTestsConfig& config);
...
TEST_F(SimpleTestCase, TESTPOOLDELETION) {
  currency = CryptAthlos::CurrencyBuilder(logger).testnet(true).mempoolTxLiveTime(60).currency();
