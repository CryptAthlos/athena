...
public:
  ...
  IntegrationTest() : 
    currency(CryptAthlos::CurrencyBuilder(log).testnet(true).currency()),
  ...
  void makeWallets() {
    for (auto& n: inodes) {
      std::unique_ptr<CryptAthlos::IWalletLegacy> wallet(new CryptAthlos::WalletLegacy(m_currency, *n));
...
TEST_F(IntegrationTest, BlockPropagationSpeed) {
  ...
  {
    std::unique_ptr<CryptAthlos::INode>& localNode = inodes.front();
    std::unique_ptr<CryptAthlos::INode>& remoteNode = inodes.back();
