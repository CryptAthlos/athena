...
public:
  ...
  CryptAthlos::TransactionId waitExternalTransaction() {
    std::unique_lock<std::mutex> lk(m_mutex);
    ...
    CryptAthlos::TransactionId txId = m_externalTransactions.front();
  ...
  std::error_code waitSendResult(CryptAthlos::TransactionId txid) {
    std::unique_lock<std::mutex> lk(m_mutex);

    std::unordered_map<CryptAthlos::TransactionId, std::error_code>::iterator it;
...
private:
  ...
  std::unordered_map<CryptAthlos::TransactionId, std::error_code> m_sendResults;
  std::deque<CryptAthlos::TransactionId> m_externalTransactions;
};
