...
InProcTestNode::InProcTestNode(const TestNodeConfiguration& cfg, const CryptAthlos::Currency& currency) :
...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    ...
    core.reset(new CryptAthlos::core(m_currency, NULL, log));
    protocol.reset(new CryptAthlos::CryptAthlosProtocolHandler(m_currency, dispatcher, *core, NULL, log));
    p2pNode.reset(new CryptAthlos::NodeServer(dispatcher, *protocol, log));
    protocol->set_p2p_endpoint(p2pNode.get());
    core->set_cryptathlos_protocol(protocol.get());

    CryptAthlos::NetNodeConfig p2pConfig;
    ...
    CryptAthlos::MinerConfig emptyMiner;
  ...
  core->deinit();
  p2pNode->deinit();
  core->set_cryptathlos_protocol(NULL);
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, CryptAthlos::Block &blockTemplate, uint64_t &difficulty) {
  AccountPublicAddress addr;
...
bool InProcTestNode::makeINode(std::unique_ptr<CryptAthlos::INode> &node) {

  std::unique_ptr<InProcessNode> inprocNode(new CryptAthlos::InProcessNode(*core, *protocol));
