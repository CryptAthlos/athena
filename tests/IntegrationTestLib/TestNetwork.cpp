...
#ifdef _WIN32
const std::string daemonExec = std::string(CryptAthlos::CRYPTATHLOS_NAME) + "d.exe";
#else
const std::string daemonExec = std::string(CryptAthlos::CRYPTATHLOS_NAME) + "d";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.testnet = testnet;
  cfg.logFile = std::string("test_") + CryptAthlos::CRYPTATHLOS_NAME + "d" + std::to_string(index) + ".log";
...
TestNetwork::TestNetwork(System::Dispatcher& dispatcher, const CryptAthlos::Currency& currency) :
