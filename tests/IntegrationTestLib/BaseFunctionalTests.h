...
namespace Tests {
  ...
  namespace po = boost::program_options;
    ...
    public:
      BaseFunctionalTests(const CryptAthlos::Currency& currency, System::Dispatcher& d, const BaseFunctionalTestsConfig& config) :
          m_dispatcher(d),
          m_currency(currency),
    ...
    protected:
      ...
      bool makeWallet(std::unique_ptr<CryptAthlos::IWalletLegacy> & wallet, std::unique_ptr<CryptAthlos::INode>& node, const std::string& password = "pass");
      bool mineBlocks(TestNode& node, const CryptAthlos::AccountPublicAddress& address, size_t blockCount);
      bool mineBlock(std::unique_ptr<CryptAthlos::IWalletLegacy>& wallet);
      bool mineBlock();
      bool startMining(size_t threads);
      bool stopMining();

      bool getNodeTransactionPool(size_t nodeIndex, CryptAthlos::INode& node, std::vector<std::unique_ptr<CryptAthlos::ITransactionReader>>& txPool);

      bool waitDaemonsReady();
      bool waitDaemonReady(size_t nodeIndex);
      bool waitForPeerCount(CryptAthlos::INode& node, size_t expectedPeerCount);
      bool waitForPoolSize(size_t nodeIndex, CryptAthlos::INode& node, size_t expectedPoolSize,
        std::vector<std::unique_ptr<CryptAthlos::ITransactionReader>>& txPool);

      bool prepareAndSubmitBlock(TestNode& node, CryptAthlos::Block&& blockTemplate);
    ...
    private:
      ...
      std::unique_ptr<CryptAthlos::INode> mainNode;
      std::unique_ptr<CryptAthlos::IWalletLegacy> workingWallet;
