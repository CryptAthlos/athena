...
public:
  InProcTestNode(const TestNodeConfiguration& cfg, const CryptAthlos::Currency& currency);
  ~InProcTestNode();
  ...
  virtual bool getBlockTemplate(const std::string &minerAddress, CryptAthlos::Block &blockTemplate, uint64_t &difficulty) override;
  virtual bool submitBlock(const std::string& block) override;
  virtual bool getTailBlockId(Crypto::Hash &tailBlockId) override;
  virtual bool makeINode(std::unique_ptr<CryptAthlos::INode>& node) override;
...
private:
  ...
  std::unique_ptr<CryptAthlos::core> core;
  std::unique_ptr<CryptAthlos::CryptAthlosProtocolHandler> protocol;
  std::unique_ptr<CryptAthlos::NodeServer> p2pNode;
