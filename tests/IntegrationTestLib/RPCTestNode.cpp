...
bool RPCTestNode::getBlockTemplate(const std::string& minerAddress, CryptAthlos::Block& blockTemplate, uint64_t& difficulty) {
...
bool RPCTestNode::makeINode(std::unique_ptr<CryptAthlos::INode>& node) {
  std::unique_ptr<CryptAthlos::INode> newNode(new CryptAthlos::NodeRpcProxy("127.0.0.1", m_rpcPort));
  NodeCallback cb;
...
uint64_t RPCTestNode::getLocalHeight() {
  try {
    CryptAthlos::COMMAND_RPC_GET_INFO::request req;
    CryptAthlos::COMMAND_RPC_GET_INFO::response rsp;
