...
#ifdef _WIN32
const std::string DAEMON_FILENAME = std::string(CRYPTATHLOS_NAME) + "d.exe";
#else
const std::string DAEMON_FILENAME = std::string(CRYPTATHLOS_NAME) + "d";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_" << CRYPTATHLOS_NAME << "d_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"" + std::string(CRYPTATHLOS_NAME) + "d" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), (std::string(CRYPTATHLOS_NAME) + "d").c_str(), "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
...
namespace {
  class WaitForCoinBaseObserver : public CryptAthlos::IWalletLegacyObserver {
    Semaphore& m_gotReward;
    CryptAthlos::IWalletLegacy& m_wallet;
  public:
    WaitForCoinBaseObserver(Semaphore& gotReward, CryptAthlos::IWalletLegacy& wallet) : m_gotReward(gotReward), m_wallet(wallet) { }
    virtual void externalTransactionCreated(CryptAthlos::TransactionId transactionId) override {
      CryptAthlos::WalletLegacyTransaction trInfo;
...
bool BaseFunctionalTests::mineBlocks(TestNode& node, const CryptAthlos::AccountPublicAddress& address, size_t blockCount) {
...
bool BaseFunctionalTests::prepareAndSubmitBlock(TestNode& node, CryptAthlos::Block&& blockTemplate) {
  blockTemplate.timestamp = m_nextTimestamp;
  m_nextTimestamp += 2 * m_currency.difficultyTarget();

  BinaryArray blockBlob = CryptAthlos::toBinaryArray(blockTemplate);
  return node.submitBlock(::Common::toHex(blockBlob.data(), blockBlob.size()));
}

bool BaseFunctionalTests::mineBlock(std::unique_ptr<CryptAthlos::IWalletLegacy> &wallet) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<CryptAthlos::IWalletLegacy> & wallet, std::unique_ptr<CryptAthlos::INode>& node, const std::string& password) {
  if (!node) return false;
  wallet = std::unique_ptr<CryptAthlos::IWalletLegacy>(new CryptAthlos::WalletLegacy(m_currency, *node));
...
namespace {
  struct PeerCountWaiter : CryptAthlos::INodeObserver {
...
bool BaseFunctionalTests::waitForPeerCount(CryptAthlos::INode& node, size_t expectedPeerCount) {
...
bool BaseFunctionalTests::waitForPoolSize(size_t nodeIndex, CryptAthlos::INode& node, size_t expectedPoolSize,
  std::vector<std::unique_ptr<CryptAthlos::ITransactionReader>>& txPool) {
...
bool BaseFunctionalTests::getNodeTransactionPool(size_t nodeIndex, CryptAthlos::INode& node,
  std::vector<std::unique_ptr<CryptAthlos::ITransactionReader>>& txPool) {
