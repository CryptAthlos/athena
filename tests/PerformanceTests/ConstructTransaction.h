...
public:
  ...
  bool test()
  {
    return CryptAthlos::constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources, m_destinations, std::vector<uint8_t>(), m_tx, 0, this->m_logger);
  }

private:
  CryptAthlos::AccountBase m_alice;
