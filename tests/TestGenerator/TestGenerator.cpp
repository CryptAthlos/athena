...
#include "CryptAthlosCore/Miner.h"
...
uint64_t test_generator::getAlreadyGeneratedCoins(const CryptAthlos::Block& blk) const {
  Crypto::Hash blkHash;
  get_block_hash(blk, blkHash);
  return getAlreadyGeneratedCoins(blkHash);
}

void test_generator::addBlock(const CryptAthlos::Block& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(CryptAthlos::Block& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
...
bool test_generator::constructBlock(CryptAthlos::Block& blk, const CryptAthlos::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<CryptAthlos::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(CryptAthlos::Block& blk, const CryptAthlos::Block& blkPrev,
                                    const CryptAthlos::AccountBase& minerAcc,
...
bool test_generator::constructBlockManuallyTx(CryptAthlos::Block& blk, const CryptAthlos::Block& prevBlock,
                                              const CryptAthlos::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(CryptAthlos::Block& blk, const CryptAthlos::Block& blkPrev,
                                           const CryptAthlos::AccountBase& minerAccount,
...
void fillNonce(CryptAthlos::Block& blk, const difficulty_type& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxBySize(const CryptAthlos::Currency& currency, CryptAthlos::Transaction& baseTransaction, uint32_t height,
  ...
  if (!currency.constructMinerTx(height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, CryptAthlos::BinaryArray(), 1)) {
    return false;
  }
