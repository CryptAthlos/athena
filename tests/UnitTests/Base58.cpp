...
{
  CryptAthlos::AccountPublicAddress addr;

  ASSERT_NO_THROW(CryptAthlos::loadFromBinary(addr, Common::asBinaryArray(test_serialized_keys)));
  std::string addr_str = CryptAthlos::getAccountAddressAsStr(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, addr);
...
TEST(parseAccountAddressString, handles_valid_address)
{
  uint64_t prefix;
  CryptAthlos::AccountPublicAddress addr;
  ASSERT_TRUE(CryptAthlos::parseAccountAddressString(prefix, addr, test_keys_addr_str));
  ASSERT_EQ(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, prefix);

  BinaryArray blob;
  ASSERT_NO_THROW(blob = CryptAthlos::storeToBinary(addr));
...
TEST(parseAccountAddressString, fails_on_invalid_address_format)
{
  ...
  uint64_t prefix;
  CryptAthlos::AccountPublicAddress addr;
  ASSERT_FALSE(CryptAthlos::parseAccountAddressString(prefix, addr, addr_str));
}
