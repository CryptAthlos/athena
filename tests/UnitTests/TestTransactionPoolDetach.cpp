...
public:
  ...
  void relayTransaction(const CryptAthlos::Transaction& transaction, const Callback& callback) override {
  ...
  void getPoolSymmetricDifference(std::vector<Crypto::Hash>&& known_pool_tx_ids, Crypto::Hash known_block_id, bool& is_bc_actual, std::vector<std::unique_ptr<CryptAthlos::ITransactionReader>>& new_txs, std::vector<Crypto::Hash>& deleted_tx_ids, const Callback& callback) override
  {
    std::unique_lock<std::mutex> lk(mutex);
    std::sort(relayedTxs.begin(), relayedTxs.end(), [](const std::pair<uint32_t, CryptAthlos::Transaction>& val1, const std::pair<uint32_t, CryptAthlos::Transaction>& val2)->bool {return val1.first < val2.first; });
    is_bc_actual = true;
    ...
    if (detached) {
      ...
      for (; i < relayedTxs.size(); ++i) {
        new_txs.push_back(CryptAthlos::createTransactionPrefix(relayedTxs[i].second));
      }
    }
  ...
  std::vector<std::pair<uint32_t, CryptAthlos::Transaction>> relayedTxs;
  uint32_t detachHeight;
  bool detached;
  std::mutex mutex;
};

class WalletSendObserver : public CryptAthlos::IWalletLegacyObserver
{
public:
  WalletSendObserver() {}
...
public:
  ...
  void generateMoneyForAccount(size_t idx) {
    generator.getBlockRewardForAddress(
      reinterpret_cast<const CryptAthlos::AccountPublicAddress&>(m_accounts[idx].address));
  }

  std::error_code submitTransaction(ITransactionReader& tx) {
    auto data = tx.getTransactionData();

    CryptAthlos::BinaryArray txblob(data.data(), data.data() + data.size());
...
struct WaitForExternalTransactionObserver : public CryptAthlos::IWalletLegacyObserver {
public:
