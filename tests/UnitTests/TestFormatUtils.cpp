...
TEST(parseTransactionExtra, handles_empty_extra)
{
  ...
  std::vector<CryptAthlos::TransactionExtraField> tx_extra_fields;
  ASSERT_TRUE(CryptAthlos::parseTransactionExtra(extra, tx_extra_fields));
...
TEST(parseTransactionExtra, handles_padding_only_size_1)
{
  ...
  ASSERT_EQ(typeid(CryptAthlos::TransactionExtraPadding), tx_extra_fields[0].type());
  ASSERT_EQ(1, boost::get<CryptAthlos::TransactionExtraPadding>(tx_extra_fields[0]).size);
}
...
TEST(parseTransactionExtra, handles_padding_only_size_2)
{
  ...
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(CryptAthlos::TransactionExtraPadding), tx_extra_fields[0].type());
  ASSERT_EQ(2, boost::get<CryptAthlos::TransactionExtraPadding>(tx_extra_fields[0]).size);
}
...
TEST(parseTransactionExtra, handles_padding_only_max_size)
{
  ...
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(CryptAthlos::TransactionExtraPadding), tx_extra_fields[0].type());
  ASSERT_EQ(TX_EXTRA_NONCE_MAX_COUNT, boost::get<CryptAthlos::TransactionExtraPadding>(tx_extra_fields[0]).size);
}
...
TEST(parseTransactionExtra, handles_padding_only_exceed_max_size)
{
  ...
  std::vector<CryptAthlos::TransactionExtraField> tx_extra_fields;
  ASSERT_FALSE(CryptAthlos::parseTransactionExtra(extra, tx_extra_fields));
}
...
TEST(parseTransactionExtra, handles_pub_key_only)
{
  ...
  ASSERT_TRUE(CryptAthlos::parseTransactionExtra(extra, tx_extra_fields));
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(CryptAthlos::TransactionExtraPublicKey), tx_extra_fields[0].type());
}
...
TEST(parseTransactionExtra, handles_extra_nonce_only)
{
  ...
  ASSERT_TRUE(CryptAthlos::parseTransactionExtra(extra, tx_extra_fields));
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(CryptAthlos::TransactionExtraNonce), tx_extra_fields[0].type());
  CryptAthlos::TransactionExtraNonce extra_nonce = boost::get<CryptAthlos::TransactionExtraNonce>(tx_extra_fields[0]);
...
TEST(parseTransactionExtra, handles_pub_key_and_padding)
{
  ...
  ASSERT_EQ(typeid(CryptAthlos::TransactionExtraPublicKey), tx_extra_fields[0].type());
  ASSERT_EQ(typeid(CryptAthlos::TransactionExtraPadding), tx_extra_fields[1].type());
}

TEST(parse_and_validate_tx_extra, is_valid_tx_extra_parsed)
{
  Logging::LoggerGroup logger;
  CryptAthlos::Currency currency = CryptAthlos::CurrencyBuilder(logger).currency();
  CryptAthlos::Transaction tx = AUTO_VAL_INIT(tx);
  CryptAthlos::AccountBase acc;
  acc.generate();
  CryptAthlos::BinaryArray b = Common::asBinaryArray("dsdsdfsdfsf");
  ASSERT_TRUE(currency.constructMinerTx(0, 0, 10000000000000, 1000, currency.minimumFee(), acc.getAccountKeys().address, tx, b, 1));
  Crypto::PublicKey tx_pub_key = CryptAthlos::getTransactionPublicKeyFromExtra(tx.extra);
  ASSERT_NE(tx_pub_key, CryptAthlos::NULL_PUBLIC_KEY);
}
TEST(parse_and_validate_tx_extra, fails_on_big_extra_nonce)
{
  ...
  CryptAthlos::AccountBase acc;
  acc.generate();
  CryptAthlos::BinaryArray b(TX_EXTRA_NONCE_MAX_COUNT + 1, 0);
...
TEST(parse_and_validate_tx_extra, fails_on_wrong_size_in_extra_nonce)
{
  ...
  tx.extra[1] = 255;
  std::vector<CryptAthlos::TransactionExtraField> tx_extra_fields;
  ASSERT_FALSE(CryptAthlos::parseTransactionExtra(tx.extra, tx_extra_fields));
}
TEST(validate_parse_amount_case, validate_parse_amount)
{
  Logging::LoggerGroup logger;
  CryptAthlos::Currency currency = CryptAthlos::CurrencyBuilder(logger).numberOfDecimalPlaces(8).currency();
  uint64_t res = 0;
