...
struct IWalletBaseStub : public CryptAthlos::IWallet {
...
protected:
  ...
  std::unique_ptr<WalletService> createWalletService(CryptAthlos::IWallet& wallet);
...
std::unique_ptr<WalletService> WalletServiceTest::createWalletService(CryptAthlos::IWallet& wallet) {
...
TEST_F(WalletServiceTest_createAddress, invalidSecretKey) {
  ...
  ASSERT_EQ(make_error_code(CryptAthlos::error::WalletServiceErrorCode::WRONG_KEY_FORMAT), ec);
}
...
TEST_F(WalletServiceTest_getTransactions, invalidAddress) {
  ...
  ASSERT_EQ(make_error_code(CryptAthlos::error::BAD_ADDRESS), ec);
}
...
TEST_F(WalletServiceTest_getTransactions, invalidPaymentId) {
  ...
  ASSERT_EQ(make_error_code(CryptAthlos::error::WalletServiceErrorCode::WRONG_PAYMENT_ID_FORMAT), ec);
}
...
TEST_F(WalletServiceTest_getTransactions, blockNotFound) {
  ...
  auto ec = service->getTransactions({}, 0, 1, "", transactions);
  ASSERT_EQ(make_error_code(CryptAthlos::error::WalletServiceErrorCode::OBJECT_NOT_FOUND), ec);
}
...
TEST_F(WalletServiceTest_getTransaction, wrongHash) {
  ...
  ASSERT_EQ(make_error_code(CryptAthlos::error::WalletServiceErrorCode::WRONG_HASH_FORMAT), ec);
}
...
public:
  virtual void SetUp() override;
protected:
  std::vector<CryptAthlos::WalletTransactionWithTransfers> transactions;
};
