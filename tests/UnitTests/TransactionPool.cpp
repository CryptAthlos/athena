...
#include "CryptAthlosCore/TransactionPool.h"
...
class TransactionValidator : public CryptAthlos::ITransactionValidator {
  virtual bool checkTransactionInputs(const CryptAthlos::Transaction& tx, BlockInfo& maxUsedBlock) override {
    return true;
  }

  virtual bool checkTransactionInputs(const CryptAthlos::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) override {
    return true;
  }

  virtual bool haveSpentKeyImages(const CryptAthlos::Transaction& tx) override {
    return false;
  }
...
public:
  ...
  TestTransactionGenerator(const CryptAthlos::Currency& currency, size_t ringSize) :
    m_currency(currency),
    m_ringSize(ringSize),
    m_miners(ringSize), 
...
namespace
{
  ...
  void GenerateTransaction(const CryptAthlos::Currency& currency, Transaction& tx, uint64_t fee, size_t outputs) {
  ...
  public:
    ...
    TestPool(const CryptAthlos::Currency& currency, Logging::ILogger& logger) :
  ...
  public:
    ...
    CryptAthlos::RealTimeProvider m_time;
...
public:
  TxPool_FillBlockTemplate() :
    tx_pool() {
    currency = CryptAthlos::CurrencyBuilder(logger).fusionTxMaxSize(TEST_FUSION_TX_MAX_SIZE).blockGrantedFullRewardZone(TEST_MEDIAN_SIZE).currency();
  }
