...
#include "CryptAthlosCore/IBlock.h"
...
ICoreStub::ICoreStub(const CryptAthlos::Block& genesisBlock) :
    topHeight(0),
    globalIndicesResult(false),
    randomOutsResult(false),
    poolTxVerificationResult(true),
...
bool ICoreStub::addObserver(CryptAthlos::ICoreObserver* observer) {
  return true;
}

bool ICoreStub::removeObserver(CryptAthlos::ICoreObserver* observer) {
  return true;
}
...
bool ICoreStub::get_random_outs_for_amounts(const CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request& req,
    CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& res) {
  res = randomOuts;
...
CryptAthlos::i_cryptathlos_protocol* ICoreStub::get_protocol() {
  return nullptr;
}

bool ICoreStub::handle_incoming_tx(CryptAthlos::BinaryArray const& tx_blob, CryptAthlos::tx_verification_context& tvc, bool keeped_by_block) {
  return true;
}
...
void ICoreStub::set_random_outs(const CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp, bool result) {
  randomOuts = resp;
  randomOutsResult = result;
}

std::vector<CryptAthlos::Transaction> ICoreStub::getPoolTransactions() {
  return std::vector<CryptAthlos::Transaction>();
}

bool ICoreStub::getPoolChanges(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
                               std::vector<CryptAthlos::Transaction>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  ...
  for (const std::pair<Crypto::Hash, CryptAthlos::Transaction>& poolEntry : transactionPool) {
...
bool ICoreStub::getPoolChangesLite(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
        std::vector<CryptAthlos::TransactionPrefixInfo>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  std::vector<CryptAthlos::Transaction> added;
  bool returnStatus = getPoolChanges(tailBlockId, knownTxsIds, added, deletedTxsIds);

  for (const auto& tx : added) {
    CryptAthlos::TransactionPrefixInfo tpi;
    tpi.txPrefix = tx;
...
void ICoreStub::getPoolChanges(const std::vector<Crypto::Hash>& knownTxsIds, std::vector<CryptAthlos::Transaction>& addedTxs,
                               std::vector<Crypto::Hash>& deletedTxsIds) {
}

bool ICoreStub::queryBlocks(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<CryptAthlos::BlockFullInfo>& entries) {
  //stub
  return true;
}

bool ICoreStub::queryBlocksLite(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<CryptAthlos::BlockShortInfo>& entries) {
  //stub
  return true;
}
...
size_t ICoreStub::addChain(const std::vector<const CryptAthlos::IBlock*>& chain) {
  size_t blocksCounter = 0;
  for (const CryptAthlos::IBlock* block : chain) {
    for (size_t txNumber = 0; txNumber < block->getTransactionCount(); ++txNumber) {
      const CryptAthlos::Transaction& tx = block->getTransaction(txNumber);
      Crypto::Hash txHash = CryptAthlos::NULL_HASH;
      size_t blobSize = 0;
...
Crypto::Hash ICoreStub::getBlockIdByHeight(uint32_t height) {
  ...
  if (iter == blockHashByHeightIndex.end()) {
    return CryptAthlos::NULL_HASH;
  }
  return iter->second;
}

bool ICoreStub::getBlockByHash(const Crypto::Hash &h, CryptAthlos::Block &blk) {
...
void ICoreStub::getTransactions(const std::vector<Crypto::Hash>& txs_ids, std::list<CryptAthlos::Transaction>& txs, std::list<Crypto::Hash>& missed_txs, bool checkTxPool) {
...
bool ICoreStub::scanOutputkeysForIndices(const CryptAthlos::KeyInput& txInToKey, std::list<std::pair<Crypto::Hash, size_t>>& outputReferences) {
  return true;
}

bool ICoreStub::getBlockDifficulty(uint32_t height, CryptAthlos::difficulty_type& difficulty) {
  return true;
}
...
bool ICoreStub::getBlockContainingTx(const Crypto::Hash& txId, Crypto::Hash& blockId, uint32_t& blockHeight) {
  ...
  blockHeight = boost::get<CryptAthlos::BaseInput>(blockIter->second.baseTransaction.inputs.front()).blockIndex;
  return true;
}

bool ICoreStub::getMultisigOutputReference(const CryptAthlos::MultisignatureInput& txInMultisig, std::pair<Crypto::Hash, size_t>& outputReference) {
  return true;
}

void ICoreStub::addBlock(const CryptAthlos::Block& block) {
  uint32_t height = boost::get<CryptAthlos::BaseInput>(block.baseTransaction.inputs.front()).blockIndex;
  Crypto::Hash hash = CryptAthlos::get_block_hash(block);
  if (height > topHeight) {
    topHeight = height;
    topId = hash;
  }
  ...
  blockHashByTxHashIndex.emplace(std::make_pair(CryptAthlos::getObjectHash(block.baseTransaction), hash));
  for (auto txHash : block.transactionHashes) {
    blockHashByTxHashIndex.emplace(std::make_pair(txHash, hash));
  }
}

void ICoreStub::addTransaction(const CryptAthlos::Transaction& tx) {
  Crypto::Hash hash = CryptAthlos::getObjectHash(tx);
...
bool ICoreStub::getOrphanBlocksByHeight(uint32_t height, std::vector<CryptAthlos::Block>& blocks) {
  return true;
}

bool ICoreStub::getBlocksByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit, std::vector<CryptAthlos::Block>& blocks, uint32_t& blocksNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getPoolTransactionsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<CryptAthlos::Transaction>& transactions, uint64_t& transactionsNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getTransactionsByPaymentId(const Crypto::Hash& paymentId, std::vector<CryptAthlos::Transaction>& transactions) {
  return true;
}
...
std::unique_ptr<CryptAthlos::IBlock> ICoreStub::getBlock(const Crypto::Hash& blockId) {
  return std::unique_ptr<CryptAthlos::IBlock>(nullptr);
}

bool ICoreStub::handleIncomingTransaction(const CryptAthlos::Transaction& tx, const Crypto::Hash& txHash, size_t blobSize, CryptAthlos::tx_verification_context& tvc, bool keptByBlock) {
...
bool ICoreStub::addMessageQueue(CryptAthlos::MessageQueue<CryptAthlos::BlockchainMessage>& messageQueuePtr) {
  return true;
}

bool ICoreStub::removeMessageQueue(CryptAthlos::MessageQueue<CryptAthlos::BlockchainMessage>& messageQueuePtr) {
  return true;
}
