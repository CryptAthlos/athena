...
public:
  ...
  void generate(const AccountPublicAddress& address, Transaction& tx, uint64_t unlockTime = 0)
  {
    ...
    CryptAthlos::decompose_amount_into_digits(this->m_source_amount, 0,
      [&](uint64_t chunk) { destinations.push_back(CryptAthlos::TransactionDestinationEntry(chunk, address)); },
      [&](uint64_t a_dust) { destinations.push_back(CryptAthlos::TransactionDestinationEntry(a_dust, address)); });

    CryptAthlos::constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources, destinations, std::vector<uint8_t>(), tx, unlockTime, m_logger);
  }
...
TestBlockchainGenerator::TestBlockchainGenerator(const CryptAthlos::Currency& currency) :
  m_currency(currency),
  generator(currency)
{
...
std::vector<CryptAthlos::Block>& TestBlockchainGenerator::getBlockchain()
{
  std::unique_lock<std::mutex> lock(m_mutex);
  return m_blockchain;
}

std::vector<CryptAthlos::Block> TestBlockchainGenerator::getBlockchainCopy() {
  std::unique_lock<std::mutex> lock(m_mutex);

  std::vector<CryptAthlos::Block> blockchain(m_blockchain);
  return blockchain;
}

bool TestBlockchainGenerator::getTransactionByHash(const Crypto::Hash& hash, CryptAthlos::Transaction& tx, bool checkTxPool)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
const CryptAthlos::AccountBase& TestBlockchainGenerator::getMinerAccount() const {
  std::unique_lock<std::mutex> lock(m_mutex);
  return miner_acc;
}
...
void TestBlockchainGenerator::addGenesisBlock() {
  ...
  m_timestampIndex.add(m_currency.genesisBlock().timestamp, CryptAthlos::get_block_hash(m_currency.genesisBlock()));
...
void TestBlockchainGenerator::addMiningBlock() {
  ...
  CryptAthlos::Block& prev_block = m_blockchain.back();
  ...
  m_timestampIndex.add(block.timestamp, CryptAthlos::get_block_hash(block));
...
void TestBlockchainGenerator::addTxToBlockchain(const CryptAthlos::Transaction& transaction)
{
  std::unique_lock<std::mutex> lock(m_mutex);
  addToBlockchain(transaction);
}

bool TestBlockchainGenerator::getBlockRewardForAddress(const CryptAthlos::AccountPublicAddress& address)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::generateTransactionsInOneBlock(const CryptAthlos::AccountPublicAddress& address, size_t n) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::getSingleOutputTransaction(const CryptAthlos::AccountPublicAddress& address, uint64_t amount) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
void TestBlockchainGenerator::addToBlockchain(const CryptAthlos::Transaction& tx) {
  addToBlockchain(std::vector<CryptAthlos::Transaction> {tx});
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<CryptAthlos::Transaction>& txs) {
  addToBlockchain(txs, miner_acc);
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<CryptAthlos::Transaction>& txs, const CryptAthlos::AccountBase& minerAddress) {
  std::list<CryptAthlos::Transaction> txsToBlock;
  ...
  std::vector<CryptAthlos::Transaction>& new_txs, std::vector<Crypto::Hash>& deleted_tx_ids)
{
  ...
  if (known_block_id != CryptAthlos::get_block_hash(m_blockchain.back())) {
    is_bc_actual = false;
    return;
  }
...
void TestBlockchainGenerator::putTxToPool(const CryptAthlos::Transaction& tx) {
  std::unique_lock<std::mutex> lock(m_mutex);

  Crypto::Hash txHash = CryptAthlos::getObjectHash(tx);
  m_txPool[txHash] = tx;
}

void TestBlockchainGenerator::putTxPoolToBlockchain() {
  std::unique_lock<std::mutex> lock(m_mutex);
  std::vector<CryptAthlos::Transaction> txs;
...
void TestBlockchainGenerator::setMinerAccount(const CryptAthlos::AccountBase& account) {
  miner_acc = account;
}
...
bool TestBlockchainGenerator::getPoolTransactionIdsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<Crypto::Hash>& hashes, uint64_t& transactionsNumberWithinTimestamps) {
  ...
  for (auto i : m_txPool) {
    ...
    hashes.push_back(CryptAthlos::getObjectHash(i.second));
    ++c;
  }
  return true;
}
...
void TestBlockchainGenerator::addTx(const CryptAthlos::Transaction& tx) {
...
bool TestBlockchainGenerator::generateFromBaseTx(const CryptAthlos::AccountBase& address) {
  std::unique_lock<std::mutex> lock(m_mutex);
