...
TEST(KVSerialize, Simple) {
  ...
  std::string buf = CryptAthlos::storeToBinaryKeyValue(testData1);
  ASSERT_TRUE(CryptAthlos::loadFromBinaryKeyValue(testData2, buf));
...
TEST(KVSerialize, BigCollection) {
  ...
  std::string buf = CryptAthlos::storeToBinaryKeyValue(ts1);
  ASSERT_TRUE(CryptAthlos::loadFromBinaryKeyValue(ts2, buf));
  EXPECT_EQ(ts1, ts2);
}
