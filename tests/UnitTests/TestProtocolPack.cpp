...
{
  std::string buff;
  CryptAthlos::NOTIFY_RESPONSE_CHAIN_ENTRY::request r;
  r.start_height = 1;
  r.total_height = 3;
  for(int i = 1; i < 10000; i += i*10) {
    r.m_block_ids.resize(i, CryptAthlos::NULL_HASH);
    buff = CryptAthlos::storeToBinaryKeyValue(r);

    CryptAthlos::NOTIFY_RESPONSE_CHAIN_ENTRY::request r2;
    CryptAthlos::loadFromBinaryKeyValue(r2, buff);
