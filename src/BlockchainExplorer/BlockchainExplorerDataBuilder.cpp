...
BlockchainExplorerDataBuilder::BlockchainExplorerDataBuilder(CryptAthlos::ICore& core, CryptAthlos::ICryptAthlosProtocolQuery& protocol) :
core(core),
protocol(protocol) {
}
...
bool BlockchainExplorerDataBuilder::fillBlockDetails(const Block &block, BlockDetails& blockDetails) {
  ...
  if (!core.getBackwardBlocksSizes(blockDetails.height, blocksSizes, parameters::CRYPTATHLOS_REWARD_BLOCKS_WINDOW)) {
    return false;
  }
