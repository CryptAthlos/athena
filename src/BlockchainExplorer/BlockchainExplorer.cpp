...
bool BlockchainExplorer::addObserver(IBlockchainObserver* observer) {
  if (state.load() != INITIALIZED) {
    throw std::system_error(make_error_code(CryptAthlos::error::BlockchainExplorerErrorCodes::NOT_INITIALIZED));
  }
...
void BlockchainExplorer::init() {
  if (state.load() != NOT_INITIALIZED) {
    ...
    throw std::system_error(make_error_code(CryptAthlos::error::BlockchainExplorerErrorCodes::ALREADY_INITIALIZED));
  }
  if (node.addObserver(this)) {
    ...
    throw std::system_error(make_error_code(CryptAthlos::error::BlockchainExplorerErrorCodes::INTERNAL_ERROR));
  }
...
uint64_t BlockchainExplorer::getRewardBlocksWindow() {
  ...
  return parameters::CRYPTATHLOS_REWARD_BLOCKS_WINDOW;
}
...
uint64_t BlockchainExplorer::getFullRewardMaxBlockSize(uint8_t majorVersion) {
  ...
  return parameters::CRYPTATHLOS_BLOCK_GRANTED_FULL_REWARD_ZONE;
}
