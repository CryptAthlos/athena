...
void SynchronizationState::save(std::ostream& os) {
  ...
  CryptAthlos::BinaryOutputStreamSerializer s(stream);
  serialize(s, "state");
}
...
CryptAthlos::ISerializer& SynchronizationState::serialize(CryptAthlos::ISerializer& s, const std::string& name) {
  s.beginObject(name);
