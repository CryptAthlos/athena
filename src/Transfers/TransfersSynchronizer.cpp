...
TransfersSyncronizer::TransfersSyncronizer(const CryptAthlos::Currency& currency, IBlockchainSynchronizer& sync, INode& node) :
...
void TransfersSyncronizer::load(std::istream& is) {
  ...
  CryptAthlos::BinaryInputStreamSerializer s(inputStream);
  uint32_t version = 0;
