...
std::vector<Crypto::Hash> getBlockHashes(const CryptAthlos::CompleteBlock* blocks, size_t count) {
...
TransfersConsumer::TransfersConsumer(const CryptAthlos::Currency& currency, INode& node, const SecretKey& viewSecret) :
...
std::error_code TransfersConsumer::onPoolUpdated(const std::vector<std::unique_ptr<ITransactionReader>>& addedTransactions, const std::vector<Hash>& deletedTransactions) {
  ...
  std::error_code processingError;
  for (auto& cryptathlosTransaction : addedTransactions) {
    m_poolTxs.emplace(cryptathlosTransaction->getTransactionHash());
    processingError = processTransaction(unconfirmedBlockInfo, *cryptathlosTransaction.get());
    if (processingError) {
...
std::error_code createTransfers(
  ...
  for (auto idx : outputs) {
    ...
    if (outType == TransactionTypes::OutputType::Key) {
      ...
      CryptAthlos::generate_key_image_helper(
        account,
        txPubKey,
        idx,
        in_ephemeral,
