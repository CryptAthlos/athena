...
const CryptAthlos::Currency PaymentGateService::getCurrency() {
...
void PaymentGateService::runInProcess(Logging::LoggerRef& log) {
  ...
  CryptAthlos::core core(currency, NULL, logger);

  CryptAthlos::CryptAthlosProtocolHandler protocol(currency, *dispatcher, core, NULL, logger);
  CryptAthlos::NodeServer p2pNode(*dispatcher, protocol, logger);

  protocol.set_p2p_endpoint(&p2pNode);
  core.set_cryptathlos_protocol(&protocol);
  ...
  std::unique_ptr<CryptAthlos::INode> node(new CryptAthlos::InProcessNode(core, protocol));
...
void PaymentGateService::runRpcProxy(Logging::LoggerRef& log) {
  ...
  std::unique_ptr<CryptAthlos::INode> node(
...
void PaymentGateService::runWalletService(const CryptAthlos::Currency& currency, CryptAthlos::INode& node) {
  PaymentService::WalletConfiguration walletConfiguration{
    config.gateConfiguration.containerFile,
    config.gateConfiguration.containerPassword
  };

  std::unique_ptr<CryptAthlos::IWallet> wallet (WalletFactory::createWallet(currency, node, *dispatcher));
