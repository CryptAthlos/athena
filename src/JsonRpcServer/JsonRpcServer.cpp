...
void JsonRpcServer::processRequest(const CryptAthlos::HttpRequest& req, CryptAthlos::HttpResponse& resp) {
  try {
    ...
    if (req.getUrl() == "/json_rpc") {
      ...
      try {
        ...
        resp.setStatus(CryptAthlos::HttpResponse::STATUS_200);
      ...
      resp.setStatus(CryptAthlos::HttpResponse::STATUS_404);
      return;
    }
  } catch (std::exception& e) {
    logger(Logging::WARNING) << "Error while processing http request: " << e.what();
    resp.setStatus(CryptAthlos::HttpResponse::STATUS_500);
  }
}
