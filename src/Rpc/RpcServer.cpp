...
RpcServer::RpcServer(System::Dispatcher& dispatcher, Logging::ILogger& log, core& c, NodeServer& p2p, const ICryptAthlosProtocolQuery& protocolQuery) :
...
bool RpcServer::onGetPoolChanges(const COMMAND_RPC_GET_POOL_CHANGES::request& req, COMMAND_RPC_GET_POOL_CHANGES::response& rsp) {
  ...
  std::vector<CryptAthlos::Transaction> addedTransactions;
...
bool RpcServer::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res) {
  ...
  CryptAthlos::BinaryArray blob_reserve;
  ...
  PublicKey tx_pub_key = CryptAthlos::getTransactionPublicKeyFromExtra(b.baseTransaction.extra);
