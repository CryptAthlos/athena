...
const char* getStatusString(CryptAthlos::HttpResponse::HTTP_STATUS status) {
  switch (status) {
  case CryptAthlos::HttpResponse::STATUS_200:
    return "200 OK";
  case CryptAthlos::HttpResponse::STATUS_404:
    return "404 Not Found";
  case CryptAthlos::HttpResponse::STATUS_500:
...
const char* getErrorBody(CryptAthlos::HttpResponse::HTTP_STATUS status) {
  switch (status) {
  case CryptAthlos::HttpResponse::STATUS_404:
...
HttpResponse::HttpResponse() {
  status = STATUS_200;
  headers["Server"] = "CryptAthlos-based HTTP server";
}
