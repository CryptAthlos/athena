...
void throwIfNotGood(std::istream& stream) {
  if (!stream.good()) {
    if (stream.eof()) {
      throw std::system_error(make_error_code(CryptAthlos::error::HttpParserErrorCodes::END_OF_STREAM));
    } else {
      throw std::system_error(make_error_code(CryptAthlos::error::HttpParserErrorCodes::STREAM_NOT_GOOD));
    }
  }
}
...
HttpResponse::HTTP_STATUS HttpParser::parseResponseStatusFromString(const std::string& status) {
  if (status == "200 OK" || status == "200 Ok") return CryptAthlos::HttpResponse::STATUS_200;
  else if (status == "404 Not Found") return CryptAthlos::HttpResponse::STATUS_404;
  else if (status == "500 Internal Server Error") return CryptAthlos::HttpResponse::STATUS_500;
  else throw std::system_error(make_error_code(CryptAthlos::error::HttpParserErrorCodes::UNEXPECTED_SYMBOL),
      "Unknown HTTP status code is given");

  return CryptAthlos::HttpResponse::STATUS_200; //unaccessible
}
...
void HttpParser::readWord(std::istream& stream, std::string& word) {
  ...
  if (c == '\r') {
    stream.get(c);
    if (c != '\n') {
      throw std::system_error(make_error_code(CryptAthlos::error::HttpParserErrorCodes::UNEXPECTED_SYMBOL));
    }
  }
}
...
bool HttpParser::readHeader(std::istream& stream, std::string& name, std::string& value) {
  ...
  while (stream.good() && c != '\r') {
    if (c == ':') {
      ...
      if (name.empty()) {
        throw std::system_error(make_error_code(CryptAthlos::error::HttpParserErrorCodes::EMPTY_HEADER));
      }
