...
template<> 
struct hash<CryptAthlos::TransactionOutputId> {
  size_t operator()(const CryptAthlos::TransactionOutputId &_v) const {
...
public:
  ...
  bool serialize(CryptAthlos::ISerializer& s);

  bool findTransactionId(const Crypto::Hash& hash, TransactionId& id);
  void erase(const Crypto::Hash& hash);
  void add(const CryptAthlos::Transaction& tx, TransactionId transactionId,
