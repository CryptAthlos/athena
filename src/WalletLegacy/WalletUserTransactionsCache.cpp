...
bool WalletUserTransactionsCache::serialize(CryptAthlos::ISerializer& s) {
...
void WalletUserTransactionsCache::updateTransaction(
  TransactionId transactionId, const CryptAthlos::Transaction& tx, uint64_t amount, const std::list<TransactionOutputInformation>& usedOutputs) {
...
std::shared_ptr<WalletLegacyEvent> WalletUserTransactionsCache::onTransactionUpdated(const TransactionInformation& txInfo, int64_t txBalance) {
  ...
  TransactionId id = CryptAthlos::WALLET_LEGACY_INVALID_TRANSACTION_ID;
  ...
  if (id == CryptAthlos::WALLET_LEGACY_INVALID_TRANSACTION_ID) {
    WalletLegacyTransaction transaction;
...
TransactionId WalletUserTransactionsCache::findTransactionByHash(const Hash& hash) {
  ...
  if (it == m_transactions.end())
    return CryptAthlos::WALLET_LEGACY_INVALID_TRANSACTION_ID;
