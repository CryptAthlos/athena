...
{
public:
  ContextCounterHolder(CryptAthlos::WalletAsyncContextCounter& shutdowner) : m_shutdowner(shutdowner) {}
  ~ContextCounterHolder() { m_shutdowner.delAsyncContext(); }

private:
  CryptAthlos::WalletAsyncContextCounter& m_shutdowner;
};
...
class InitWaiter : public CryptAthlos::IWalletLegacyObserver {
public:
...
class SaveWaiter : public CryptAthlos::IWalletLegacyObserver {
public:
...
class SyncStarter : public CryptAthlos::IWalletLegacyObserver {
public:
...
WalletLegacy::WalletLegacy(const CryptAthlos::Currency& currency, INode& node) :
...
void WalletLegacy::doLoad(std::istream& source) {
  ...
  try {
    ...
    runAtomic(m_cacheMutex, [this] () {this->m_state = WalletLegacy::NOT_INITIALIZED;} );
    m_observerManager.notify(&IWalletLegacyObserver::initCompleted, make_error_code(CryptAthlos::error::INTERNAL_WALLET_ERROR));
    return;
  }
...
void WalletLegacy::save(std::ostream& destination, bool saveDetailed, bool saveCache) {
  if(m_isStopping) {
    m_observerManager.notify(&IWalletLegacyObserver::saveCompleted, make_error_code(CryptAthlos::error::OPERATION_CANCELLED));
    return;
  }

  {
    ...
    throwIf(m_state != INITIALIZED, CryptAthlos::error::WRONG_STATE);
...
void WalletLegacy::doSave(std::ostream& destination, bool saveDetailed, bool saveCache) {
  ...
  try {
    ...
    runAtomic(m_cacheMutex, [this] () {this->m_state = WalletLegacy::INITIALIZED;} );
    m_observerManager.notify(&IWalletLegacyObserver::saveCompleted, make_error_code(CryptAthlos::error::INTERNAL_WALLET_ERROR));
    return;
  }
...
std::error_code WalletLegacy::changePassword(const std::string& oldPassword, const std::string& newPassword) {
  ...
  if (m_password.compare(oldPassword))
    return make_error_code(CryptAthlos::error::WRONG_PASSWORD);
...
std::error_code WalletLegacy::cancelTransaction(size_t transactionId) {
  return make_error_code(CryptAthlos::error::TX_CANCEL_IMPOSSIBLE);
}
