...
std::error_code walletSaveWrapper(CryptAthlos::IWalletLegacy& wallet, std::ofstream& file, bool saveDetailes, bool saveCache) {
  CryptAthlos::WalletHelper::SaveWalletResultObserver o;
...
void WalletHelper::SendCompleteResultObserver::sendTransactionCompleted(CryptAthlos::TransactionId transactionId, std::error_code result) {
  std::lock_guard<std::mutex> lock(m_mutex);
  m_finishedTransactions[transactionId] = result;
  m_condition.notify_one();
}

std::error_code WalletHelper::SendCompleteResultObserver::wait(CryptAthlos::TransactionId transactionId) {
...
WalletHelper::IWalletRemoveObserverGuard::IWalletRemoveObserverGuard(CryptAthlos::IWalletLegacy& wallet, CryptAthlos::IWalletLegacyObserver& observer) :
  m_wallet(wallet),
  m_observer(observer),
  m_removed(false) {
...
void WalletHelper::storeWallet(CryptAthlos::IWalletLegacy& wallet, const std::string& walletFilename) {
