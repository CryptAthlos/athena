...
WalletLegacySerializer::WalletLegacySerializer(CryptAthlos::AccountBase& account, WalletUserTransactionsCache& transactionsCache) :
  account(account),
...
void WalletLegacySerializer::serialize(std::ostream& stream, const std::string& password, bool saveDetailed, const std::string& cache) {
  ...
  CryptAthlos::BinaryOutputStreamSerializer serializer(plainStream);
  saveKeys(serializer);
...
void WalletLegacySerializer::saveKeys(CryptAthlos::ISerializer& serializer) {
  CryptAthlos::KeysStorage keys;
  CryptAthlos::AccountKeys acc = account.getAccountKeys();
...
void WalletLegacySerializer::deserialize(std::istream& stream, const std::string& password, std::string& cache) {
  ...
  CryptAthlos::BinaryInputStreamSerializer serializerEncrypted(stdStream);
...
void WalletLegacySerializer::loadKeys(CryptAthlos::ISerializer& serializer) {
  CryptAthlos::KeysStorage keys;

  keys.serialize(serializer, "keys");

  CryptAthlos::AccountKeys acc;
