  ...
  if (result != 0) {
    if (result == 1) {
      ...
      if (UPNP_AddPortMapping(urls.controlURL, igdData.first.servicetype, portString.str().c_str(),
        portString.str().c_str(), lanAddress, CryptAthlos::CRYPTATHLOS_NAME, "TCP", 0, "0") != 0) {
...
namespace CryptAthlos
{
  namespace
  {
    ...
    const command_line::arg_descriptor<std::string> arg_p2p_bind_port      = {"p2p-bind-port", "Port for p2p network protocol", std::to_string(CryptAthlos::P2P_DEFAULT_PORT)};
  ...
  NodeServer::NodeServer(System::Dispatcher& dispatcher, CryptAthlos::CryptAthlosProtocolHandler& payload_handler, Logging::ILogger& log) :
    m_dispatcher(dispatcher),
    m_workingContextGroup(dispatcher),
    m_payload_handler(payload_handler),
    m_allow_local_ip(false),
    m_hide_my_port(false),
    m_network_id(CRYPTATHLOS_NETWORK),
    ...
    m_stop(false),
    // intervals
    // m_peer_handshake_idle_maker_interval(CryptAthlos::P2P_DEFAULT_HANDSHAKE_INTERVAL),
  ...
  int NodeServer::handleCommand(const LevinProtocol::Command& cmd, BinaryArray& out, P2pConnectionContext& ctx, bool& handled) {
    ...
    if (cmd.isResponse && cmd.command == COMMAND_TIMED_SYNC::ID) {
      if (!handleTimedSyncResponse(cmd.buf, ctx)) {
        ...
        ctx.m_state = CryptAthlosConnectionContext::state_shutdown;
      }
      return 0;
    }
...
#undef INVOKE_HANDLER
  ...
  bool NodeServer::init_config() {
    try {
      ...
      m_config.m_net_config.handshake_interval = CryptAthlos::P2P_DEFAULT_HANDSHAKE_INTERVAL;
      m_config.m_net_config.connections_count = CryptAthlos::P2P_DEFAULT_CONNECTIONS_COUNT;
      m_config.m_net_config.packet_max_size = CryptAthlos::P2P_DEFAULT_PACKET_MAX_SIZE; //20 MB limit
      m_config.m_net_config.config_id = 0; // initial config
      m_config.m_net_config.connection_timeout = CryptAthlos::P2P_DEFAULT_CONNECTION_TIMEOUT;
      m_config.m_net_config.ping_connection_timeout = CryptAthlos::P2P_DEFAULT_PING_CONNECTION_TIMEOUT;
      m_config.m_net_config.send_peerlist_sz = CryptAthlos::P2P_DEFAULT_PEERS_IN_HANDSHAKE;
  ...
  //----------------------------------------------------------------------------------- 
  void NodeServer::for_each_connection(std::function<void(CryptAthlosConnectionContext&, PeerIdType)> f)
  {
  ...
  bool NodeServer::init(const NetNodeConfig& config) {
    if (!config.getTestnet()) {
      for (auto seed : CryptAthlos::SEED_NODES) {
    ...
    // m_net_server.get_config_object().m_invoke_timeout = CryptAthlos::P2P_DEFAULT_INVOKE_TIMEOUT;
  ...
  CryptAthlos::CryptAthlosProtocolHandler& NodeServer::get_payload_object()
  {
  ...
  //----------------------------------------------------------------------------------- 
  bool NodeServer::handshake(CryptAthlos::LevinProtocol& proto, P2pConnectionContext& context, bool just_take_peerlist) {
  ...
  bool NodeServer::timedSync() {
    ...
    forEachConnection([&](P2pConnectionContext& conn) {
      if (conn.peerId && 
          (conn.m_state == CryptAthlosConnectionContext::state_normal ||
           conn.m_state == CryptAthlosConnectionContext::state_idle)) {
    ...
    try {
      ...
      try {
        System::Context<bool> handshakeContext(m_dispatcher, [&] {
          CryptAthlos::LevinProtocol proto(ctx.connection);
  ...
  bool NodeServer::connections_maker()
  {
    ...
    size_t expected_white_connections = (m_config.m_net_config.connections_count * CryptAthlos::P2P_DEFAULT_WHITELIST_CONNECTIONS_PERCENT) / 100;
  ...
  bool NodeServer::handle_remote_peerlist(const std::list<PeerlistEntry>& peerlist, time_t local_time, const CryptAthlosConnectionContext& context)
  {
    int64_t delta = 0;
  ...
  bool NodeServer::check_trust(const proof_of_trust &tr) {
    ...
    Crypto::PublicKey pk;
    Common::podFromHex(CryptAthlos::P2P_STAT_TRUSTED_PUB_KEY, pk);
  ...
  void NodeServer::relay_notify_to_all(int command, const BinaryArray& data_buff, const net_connection_id* excludeConnection) {
    ...
    forEachConnection([&](P2pConnectionContext& conn) {
      if (conn.peerId && conn.m_connection_id != excludeId &&
          (conn.m_state == CryptAthlosConnectionContext::state_normal ||
           conn.m_state == CryptAthlosConnectionContext::state_synchronizing)) {
  ...
  //-----------------------------------------------------------------------------------
  bool NodeServer::invoke_notify_to_peer(int command, const BinaryArray& buffer, const CryptAthlosConnectionContext& context) {
  ...
  void NodeServer::connectionHandler(const boost::uuids::uuid& connectionId, P2pConnectionContext& ctx) {
    ...
    System::Context<> context(m_dispatcher, [this, &connectionId, &ctx] {
      ...
      try {
        ...
        for (;;) {
          if (ctx.m_state == CryptAthlosConnectionContext::state_sync_required) {
            ctx.m_state = CryptAthlosConnectionContext::state_synchronizing;
            m_payload_handler.start_sync(ctx);
          } else if (ctx.m_state == CryptAthlosConnectionContext::state_pool_sync_required) {
            ctx.m_state = CryptAthlosConnectionContext::state_normal;
          ...
          if (ctx.m_state == CryptAthlosConnectionContext::state_shutdown) {
            break;
          }
        }
      } catch (System::InterruptedException&) {
