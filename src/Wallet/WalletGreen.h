...
protected:
  ...
  struct ReceiverAmounts {
    CryptAthlos::AccountPublicAddress receiver;
  ...
  void updateBalance(CryptAthlos::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(CryptAthlos::ITransfersContainer* container) const;

  CryptAthlos::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const CryptAthlos::AccountPublicAddress& changeDestinationAddress,
  ...
  std::unique_ptr<CryptAthlos::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp);

  void sendTransaction(const CryptAthlos::Transaction& cryptAthlosTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const CryptAthlos::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, CryptAthlos::ITransfersContainer* container);
  ...
  CryptAthlos::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;
