...
void parseAddressString(const std::string& string, const CryptAthlos::Currency& currency, CryptAthlos::AccountPublicAddress& address) {
  if (!currency.parseAccountAddressString(string, address)) {
    throw std::system_error(make_error_code(CryptAthlos::error::BAD_ADDRESS));
  }
}

void validateAddresses(const std::vector<std::string>& addresses, const CryptAthlos::Currency& currency) {
  for (const auto& address: addresses) {
    if (!CryptAthlos::validateAddress(address, currency)) {
      throw std::system_error(make_error_code(CryptAthlos::error::BAD_ADDRESS));
    }
  }
}

void validateOrders(const std::vector<WalletOrder>& orders, const CryptAthlos::Currency& currency) {
  for (const auto& order: orders) {
    if (!CryptAthlos::validateAddress(order.address, currency)) {
      throw std::system_error(make_error_code(CryptAthlos::error::BAD_ADDRESS));
    }

    if (order.amount >= static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
      throw std::system_error(make_error_code(CryptAthlos::error::WRONG_AMOUNT),
        "Order amount must not exceed " + std::to_string(std::numeric_limits<int64_t>::max()));
    }
  }
}

uint64_t countNeededMoney(const std::vector<CryptAthlos::WalletTransfer>& destinations, uint64_t fee) {
  uint64_t neededMoney = 0;
  for (const auto& transfer: destinations) {
    if (transfer.amount == 0) {
      throw std::system_error(make_error_code(CryptAthlos::error::ZERO_DESTINATION));
    ...
    if (neededMoney < uamount) {
      throw std::system_error(make_error_code(CryptAthlos::error::SUM_OVERFLOW));
    }
  }
...
void checkIfEnoughMixins(std::vector<CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult, uint64_t mixIn) {
  auto notEnoughIt = std::find_if(mixinResult.begin(), mixinResult.end(),
    [mixIn] (const CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount& ofa) { return ofa.outs.size() < mixIn; } );

  if (mixIn == 0 && mixinResult.empty()) {
    throw std::system_error(make_error_code(CryptAthlos::error::MIXIN_COUNT_TOO_BIG));
  }
...
CryptAthlos::WalletEvent makeTransactionUpdatedEvent(size_t id) {
  CryptAthlos::WalletEvent event;
  event.type = CryptAthlos::WalletEventType::TRANSACTION_UPDATED;
  event.transactionUpdated.transactionIndex = id;

  return event;
}

CryptAthlos::WalletEvent makeTransactionCreatedEvent(size_t id) {
  CryptAthlos::WalletEvent event;
  event.type = CryptAthlos::WalletEventType::TRANSACTION_CREATED;
  event.transactionCreated.transactionIndex = id;

  return event;
}

CryptAthlos::WalletEvent makeMoneyUnlockedEvent() {
  CryptAthlos::WalletEvent event;
  event.type = CryptAthlos::WalletEventType::BALANCE_UNLOCKED;

  return event;
}

CryptAthlos::WalletEvent makeSyncProgressUpdatedEvent(uint32_t current, uint32_t total) {
  CryptAthlos::WalletEvent event;
  event.type = CryptAthlos::WalletEventType::SYNC_PROGRESS_UPDATED;
...
CryptAthlos::WalletEvent makeSyncCompletedEvent() {
  CryptAthlos::WalletEvent event;
  event.type = CryptAthlos::WalletEventType::SYNC_COMPLETED;
...
CryptAthlos::AccountPublicAddress parseAccountAddressString(const std::string& addressString, const CryptAthlos::Currency& currency) {
  CryptAthlos::AccountPublicAddress address;
...
void WalletGreen::initializeWithViewKey(const Crypto::SecretKey& viewSecretKey, const std::string& password) {
  ...
  if (!Crypto::secret_key_to_public_key(viewSecretKey, viewPublicKey)) {
    throw std::system_error(make_error_code(CryptAthlos::error::KEY_GENERATION_ERROR));
  }
...
void WalletGreen::initWithKeys(const Crypto::PublicKey& viewPublicKey, const Crypto::SecretKey& viewSecretKey, const std::string& password) {
  if (m_state != WalletState::NOT_INITIALIZED) {
    throw std::system_error(make_error_code(CryptAthlos::error::ALREADY_INITIALIZED));
  }
...
KeyPair WalletGreen::getAddressSpendKey(const std::string& address) const {
  ...
  CryptAthlos::AccountPublicAddress pubAddr = parseAddress(address);
...
WalletTransaction WalletGreen::getTransaction(size_t transactionIndex) const {
  ...
  if (m_transactions.size() <= transactionIndex) {
    throw std::system_error(make_error_code(CryptAthlos::error::INDEX_OUT_OF_RANGE));
  }
...
void WalletGreen::prepareTransaction(std::vector<WalletOuts>&& wallets,
  ...
  const CryptAthlos::AccountPublicAddress& changeDestination,
  ...
  typedef CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount outs_for_amount;
...
void WalletGreen::validateTransactionParameters(const TransactionParameters& transactionParameters) {
  ...
  if (transactionParameters.changeDestination.empty()) {
    ...
    if (!CryptAthlos::validateAddress(transactionParameters.changeDestination, m_currency)) {
      throw std::system_error(make_error_code(CryptAthlos::error::BAD_ADDRESS), "Wrong change address");
    }
...
size_t WalletGreen::doTransfer(const TransactionParameters& transactionParameters) {
  ...
  CryptAthlos::AccountPublicAddress changeDestination = getChangeDestination(transactionParameters.changeDestination, transactionParameters.sourceAddresses);
...
size_t WalletGreen::makeTransaction(const TransactionParameters& sendingTransaction) {
  ...
  CryptAthlos::AccountPublicAddress changeDestination = getChangeDestination(sendingTransaction.changeDestination, sendingTransaction.sourceAddresses);
...
size_t WalletGreen::insertOutgoingTransactionAndPushEvent(const Hash& transactionHash, uint64_t fee, const BinaryArray& extra, uint64_t unlockTimestamp) {
  ...
  insertTx.blockHeight = CryptAthlos::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT;
...
bool WalletGreen::updateWalletTransactionInfo(size_t transactionId, const CryptAthlos::TransactionInformation& info, int64_t totalAmount) {
...
std::unique_ptr<CryptAthlos::ITransaction> WalletGreen::makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
...
void WalletGreen::sendTransaction(const CryptAthlos::Transaction& cryptAthlosTransaction) {
  System::Event completion(m_dispatcher);
  std::error_code ec;

  throwIfStopped();
  m_node.relayTransaction(cryptAthlosTransaction, [&ec, &completion, this](std::error_code error) {
    ec = error;
    this->m_dispatcher.remoteSpawn(std::bind(asyncRequestCompletion, std::ref(completion)));
  });
  completion.wait();
...
size_t WalletGreen::validateSaveAndSendTransaction(const ITransactionReader& transaction, const std::vector<WalletTransfer>& destinations, bool isFusion, bool send) {
  ...
  CryptAthlos::Transaction cryptAthlosTransaction;
  if (!fromBinaryArray(cryptAthlosTransaction, transactionData)) {
  ...
  if (send) {
    sendTransaction(cryptAthlosTransaction);
    updateTransactionStateAndPushEvent(transactionId, WalletTransactionState::SUCCEEDED);
  } else {
    assert(m_uncommitedTransactions.count(transactionId) == 0);
    m_uncommitedTransactions.emplace(transactionId, std::move(cryptAthlosTransaction));
  }
...
void WalletGreen::requestMixinOuts(
  ...
  uint64_t mixIn,
  std::vector<CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult) {
...
std::vector<CryptAthlos::WalletGreen::ReceiverAmounts> WalletGreen::splitDestinations(const std::vector<CryptAthlos::WalletTransfer>& destinations,
  uint64_t dustThreshold,
...
CryptAthlos::WalletGreen::ReceiverAmounts WalletGreen::splitAmount(
  uint64_t amount,
...
void WalletGreen::prepareInputs(
  ...
  typedef CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::out_entry out_entry;
...
void WalletGreen::throwIfNotInitialized() const {
  if (m_state != WalletState::INITIALIZED) {
    throw std::system_error(make_error_code(CryptAthlos::error::NOT_INITIALIZED));
  }
}
...
void WalletGreen::transactionUpdated(const TransactionInformation& transactionInfo, const std::vector<ContainerAmounts>& containerAmountsList) {
  ...
  if (transactionInfo.blockHeight != CryptAthlos::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT) {
...
void WalletGreen::transactionDeleted(ITransfersSubscription* object, const Hash& transactionHash) {
  ...
  CryptAthlos::ITransfersContainer* container = &object->getContainer();
  updateBalance(container);
  deleteUnlockTransactionJob(transactionHash);

  bool updated = false;
  m_transactions.get<TransactionIndex>().modify(it, [&updated](CryptAthlos::WalletTransaction& tx) {
...
void WalletGreen::insertUnlockTransactionJob(const Hash& transactionHash, uint32_t blockHeight, CryptAthlos::ITransfersContainer* container) {
  auto& index = m_unlockTransactionsJob.get<BlockHeightIndex>();
...
void WalletGreen::updateBalance(CryptAthlos::ITransfersContainer* container) {
...
const WalletRecord& WalletGreen::getWalletRecord(CryptAthlos::ITransfersContainer* container) const {
  auto it = m_walletsContainer.get<TransfersContainerIndex>().find(container);
  if (it == m_walletsContainer.get<TransfersContainerIndex>().end()) {
    throw std::system_error(make_error_code(error::WALLET_NOT_FOUND));
  }
...
CryptAthlos::AccountPublicAddress WalletGreen::parseAddress(const std::string& address) const {
...
CryptAthlos::AccountPublicAddress WalletGreen::getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const {
...
bool WalletGreen::isMyAddress(const std::string& addressString) const {
  CryptAthlos::AccountPublicAddress address = parseAccountAddressString(addressString, m_currency);
