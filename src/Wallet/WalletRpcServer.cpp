...
void wallet_rpc_server::processRequest(const CryptAthlos::HttpRequest& request, CryptAthlos::HttpResponse& response) {

  using namespace CryptAthlos::JsonRpc;
...
bool wallet_rpc_server::on_transfer(const wallet_rpc::COMMAND_RPC_TRANSFER::request& req, wallet_rpc::COMMAND_RPC_TRANSFER::response& res) {
  ...
  if (!req.payment_id.empty()) {
    ...
    if (!CryptAthlos::parsePaymentId(payment_id_str, payment_id)) {
    ...
    CryptAthlos::setPaymentIdToTransactionExtraNonce(extra_nonce, payment_id);
    if (!CryptAthlos::addExtraNonceToTransactionExtra(extra, extra_nonce)) {
      throw JsonRpc::JsonRpcError(WALLET_RPC_ERROR_CODE_WRONG_PAYMENT_ID,
  ...
  try {
    ...
    CryptAthlos::TransactionId tx = m_wallet.sendTransaction(transfers, req.fee, extraString, req.mixin, req.unlock_time);
...
bool wallet_rpc_server::on_get_payments(const wallet_rpc::COMMAND_RPC_GET_PAYMENTS::request& req, wallet_rpc::COMMAND_RPC_GET_PAYMENTS::response& res) {
  ...
  CryptAthlos::BinaryArray payment_id_blob;
