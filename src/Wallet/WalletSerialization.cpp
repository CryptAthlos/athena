...
#include "CryptAthlosCore/CryptAthlosSerialization.h"
...
struct WalletTransactionDto {
  ...
  WalletTransactionDto(const CryptAthlos::WalletTransaction& wallet) {
    state = wallet.state;
  ...
  CryptAthlos::WalletTransactionState state;
  uint64_t timestamp;
  uint32_t blockHeight;
  Hash hash;
  int64_t totalAmount;
...
struct WalletTransferDto {
  ...
  WalletTransferDto(const CryptAthlos::WalletTransfer& tr, uint32_t version) : WalletTransferDto(version) {
    address = tr.address;
    amount = tr.amount;
...
void serialize(WalletRecordDto& value, CryptAthlos::ISerializer& serializer) {
...
void serialize(ObsoleteSpentOutputDto& value, CryptAthlos::ISerializer& serializer) {
...
void serialize(ObsoleteChangeDto& value, CryptAthlos::ISerializer& serializer) {
  serializer(value.txHash, "transaction_hash");
  serializer(value.amount, "amount");
}

void serialize(UnlockTransactionJobDto& value, CryptAthlos::ISerializer& serializer) {
  serializer(value.blockHeight, "block_height");
  serializer(value.transactionHash, "transaction_hash");
  serializer(value.walletIndex, "wallet_index");
}

void serialize(WalletTransactionDto& value, CryptAthlos::ISerializer& serializer) {
  typedef std::underlying_type<CryptAthlos::WalletTransactionState>::type StateType;

  StateType state = static_cast<StateType>(value.state);
  serializer(state, "state");
  value.state = static_cast<CryptAthlos::WalletTransactionState>(state);

  serializer(value.timestamp, "timestamp");
  CryptAthlos::serializeBlockHeight(serializer, value.blockHeight, "block_height");
...
void serialize(WalletTransferDto& value, CryptAthlos::ISerializer& serializer) {
...
std::string serialize(Object& obj, const std::string& name) {
  ...
  CryptAthlos::BinaryOutputStreamSerializer s(output);
...
std::string encrypt(const std::string& plain, CryptAthlos::CryptoContext& cryptoContext) {
  std::string cipher;
...
void addToStream(const std::string& cipher, const std::string& name, Common::IOutputStream& destination) {
  CryptAthlos::BinaryOutputStreamSerializer s(destination);
  s(const_cast<std::string& >(cipher), name);
}

template<typename Object>
void serializeEncrypted(Object& obj, const std::string& name, CryptAthlos::CryptoContext& cryptoContext, Common::IOutputStream& destination) {
...
std::string readCipher(Common::IInputStream& source, const std::string& name) {
  std::string cipher;
  CryptAthlos::BinaryInputStreamSerializer s(source);
  s(cipher, name);

  return cipher;
}

std::string decrypt(const std::string& cipher, CryptAthlos::CryptoContext& cryptoContext) {
  std::string plain;
...
void deserialize(Object& obj, const std::string& name, const std::string& plain) {
  ...
  CryptAthlos::BinaryInputStreamSerializer s(stream);
  s(obj, Common::StringView(name));
}

template<typename Object>
void deserializeEncrypted(Object& obj, const std::string& name, CryptAthlos::CryptoContext& cryptoContext, Common::IInputStream& source) {
...
CryptAthlos::WalletTransaction convert(const CryptAthlos::WalletLegacyTransaction& tx) {
  CryptAthlos::WalletTransaction mtx;

  mtx.state = CryptAthlos::WalletTransactionState::SUCCEEDED;
...
CryptAthlos::WalletTransfer convert(const CryptAthlos::WalletLegacyTransfer& tr) {
  CryptAthlos::WalletTransfer mtr;
...
void WalletSerializer::loadWallet(Common::IInputStream& source, const std::string& password, uint32_t version) {
  CryptAthlos::CryptoContext cryptoContext;
...
void WalletSerializer::loadWalletV1(Common::IInputStream& source, const std::string& password) {
  ...
  CryptAthlos::BinaryInputStreamSerializer encrypted(source);
  ...
  CryptAthlos::BinaryInputStreamSerializer serializer(decryptedStream);
...
void WalletSerializer::loadWalletV1Keys(CryptAthlos::BinaryInputStreamSerializer& serializer) {
  CryptAthlos::KeysStorage keys;
...
void WalletSerializer::loadWalletV1Details(CryptAthlos::BinaryInputStreamSerializer& serializer) {
...
void WalletSerializer::loadWallets(Common::IInputStream& source, CryptoContext& cryptoContext) {
  ...
  for (uint64_t i = 0; i < count; ++i) {
    ...
    wallet.container = reinterpret_cast<CryptAthlos::ITransfersContainer*>(i); //dirty hack. container field must be unique
