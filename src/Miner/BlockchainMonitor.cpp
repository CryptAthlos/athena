...
Crypto::Hash BlockchainMonitor::requestLastBlockHash() {
  ...
  try {
    CryptAthlos::HttpClient client(m_dispatcher, m_daemonHost, m_daemonPort);

    CryptAthlos::COMMAND_RPC_GET_LAST_BLOCK_HEADER::request request;
    CryptAthlos::COMMAND_RPC_GET_LAST_BLOCK_HEADER::response response;

    System::EventLock lk(m_httpEvent);
    CryptAthlos::JsonRpc::invokeJsonRpcCommand(client, "getlastblockheader", request, response);
