...
class NodeRpcProxy : public CryptAthlos::INode {
public:
  ...
  virtual bool addObserver(CryptAthlos::INodeRpcProxyObserver* observer);
  virtual bool removeObserver(CryptAthlos::INodeRpcProxyObserver* observer);
...
private:
  ...
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>& knownBlockIds,
    std::vector<CryptAthlos::block_complete_entry>& newBlocks, uint32_t& startHeight);
...
private:
  ...
  Tools::ObserverManager<CryptAthlos::INodeObserver> m_observerManager;
  Tools::ObserverManager<CryptAthlos::INodeRpcProxyObserver> m_rpcProxyObserverManager;
