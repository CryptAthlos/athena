...
#include "CryptAthlosCore/IMinerHandler.h"
...
namespace CryptAthlos {
  ...
  class core : public ICore, public IMinerHandler, public IBlockchainStorageObserver, public ITxPoolObserver {
   public:
     core(const Currency& currency, i_cryptathlos_protocol* pprotocol, Logging::ILogger& logger);
     ~core();

     bool on_idle() override;
     virtual bool handle_incoming_tx(const BinaryArray& tx_blob, tx_verification_context& tvc, bool keeped_by_block) override; //Deprecated. Should be removed with CryptAthlosProtocolHandler.
     ...
     virtual i_cryptathlos_protocol* get_protocol() override {return m_pprotocol;}
     ...
     virtual bool handle_get_objects(NOTIFY_REQUEST_GET_OBJECTS_request& arg, NOTIFY_RESPONSE_GET_OBJECTS_request& rsp) override; //Deprecated. Should be removed with CryptAthlosProtocolHandler.
    ...
    virtual Crypto::Hash getBlockIdByHeight(uint32_t height) override;
     ...
     void set_cryptathlos_protocol(i_cryptathlos_protocol* pprotocol);
   ...
   private:
     ...
     CryptAthlos::RealTimeProvider m_timeProvider;
     tx_memory_pool m_mempool;
     Blockchain m_blockchain;
     i_cryptathlos_protocol* m_pprotocol;
     std::unique_ptr<miner> m_miner;
     std::string m_config_folder;
     cryptathlos_protocol_stub m_protocol_stub;
