...
core::core(const Currency& currency, i_cryptathlos_protocol* pprotocol, Logging::ILogger& logger) :
m_currency(currency),
logger(logger, "core"),
...
m_starter_message_showed(false) {
  set_cryptathlos_protocol(pprotocol);
...
void core::set_cryptathlos_protocol(i_cryptathlos_protocol* pprotocol) {
  if (pprotocol)
...
bool core::handle_incoming_tx(const BinaryArray& tx_blob, tx_verification_context& tvc, bool keeped_by_block) { //Deprecated. Should be removed with CryptAthlosProtocolHandler.
...
bool core::handle_get_objects(NOTIFY_REQUEST_GET_OBJECTS::request& arg, NOTIFY_RESPONSE_GET_OBJECTS::request& rsp) { //Deprecated. Should be removed with CryptAthlosProtocolHandler.
