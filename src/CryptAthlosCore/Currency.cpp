...
#include "Account.h"
#include "CryptAthlosBasicImpl.h"
...
bool Currency::parseAccountAddressString(const std::string& str, AccountPublicAddress& addr) const {
  uint64_t prefix;
  if (!CryptAthlos::parseAccountAddressString(prefix, addr, str)) {
    return false;
  }
...
CurrencyBuilder::CurrencyBuilder(Logging::ILogger& log) : m_currency(log) {
  maxBlockNumber(parameters::CRYPTATHLOS_MAX_BLOCK_NUMBER);
  maxBlockBlobSize(parameters::CRYPTATHLOS_MAX_BLOCK_BLOB_SIZE);
  maxTxSize(parameters::CRYPTATHLOS_MAX_TX_SIZE);
  publicAddressBase58Prefix(parameters::CRYPTATHLOS_PUBLIC_ADDRESS_BASE58_PREFIX);
  minedMoneyUnlockWindow(parameters::CRYPTATHLOS_MINED_MONEY_UNLOCK_WINDOW);

  timestampCheckWindow(parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW);
  blockFutureTimeLimit(parameters::CRYPTATHLOS_BLOCK_FUTURE_TIME_LIMIT);

  moneySupply(parameters::MONEY_SUPPLY);
  emissionSpeedFactor(parameters::EMISSION_SPEED_FACTOR);

  rewardBlocksWindow(parameters::CRYPTATHLOS_REWARD_BLOCKS_WINDOW);
  blockGrantedFullRewardZone(parameters::CRYPTATHLOS_BLOCK_GRANTED_FULL_REWARD_ZONE);
  minerTxBlobReservedSize(parameters::CRYPTATHLOS_COINBASE_BLOB_RESERVED_SIZE);

  numberOfDecimalPlaces(parameters::CRYPTATHLOS_DISPLAY_DECIMAL_POINT);
  ...
  lockedTxAllowedDeltaSeconds(parameters::CRYPTATHLOS_LOCKED_TX_ALLOWED_DELTA_SECONDS);
  lockedTxAllowedDeltaBlocks(parameters::CRYPTATHLOS_LOCKED_TX_ALLOWED_DELTA_BLOCKS);

  mempoolTxLiveTime(parameters::CRYPTATHLOS_MEMPOOL_TX_LIVETIME);
  mempoolTxFromAltBlockLiveTime(parameters::CRYPTATHLOS_MEMPOOL_TX_FROM_ALT_BLOCK_LIVETIME);
  numberOfPeriodsToForgetTxDeletedFromPool(parameters::CRYPTATHLOS_NUMBER_OF_PERIODS_TO_FORGET_TX_DELETED_FROM_POOL);

  fusionTxMaxSize(parameters::FUSION_TX_MAX_SIZE);
  fusionTxMinInputCount(parameters::FUSION_TX_MIN_INPUT_COUNT);
  fusionTxMinInOutCountRatio(parameters::FUSION_TX_MIN_IN_OUT_COUNT_RATIO);

  blocksFileName(parameters::CRYPTATHLOS_BLOCKS_FILENAME);
  blocksCacheFileName(parameters::CRYPTATHLOS_BLOCKSCACHE_FILENAME);
  blockIndexesFileName(parameters::CRYPTATHLOS_BLOCKINDEXES_FILENAME);
  txPoolFileName(parameters::CRYPTATHLOS_POOLDATA_FILENAME);
  blockchainIndicesFileName(parameters::CRYPTATHLOS_BLOCKCHAIN_INDICES_FILENAME);
...
Transaction CurrencyBuilder::generateGenesisTransaction() {
  ...
  CryptAthlos::AccountPublicAddress ac = boost::value_initialized<CryptAthlos::AccountPublicAddress>();
