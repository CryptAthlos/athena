...
namespace CryptAthlos
{
  ...
  bool miner::request_block_template() {
    ...
    uint32_t height;
    CryptAthlos::BinaryArray extra_nonce;
  ...
  bool miner::init(const MinerConfig& config) {
    if (!config.extraMessages.empty()) {
      ...
      std::string filebuf;
      if (Common::loadFileToString(m_config_folder_path + "/" + CryptAthlos::parameters::MINER_CONFIG_FILE_NAME, filebuf)) {
    ...
    while(!m_stop)
    {
      ...
      if (!m_stop && check_hash(h, local_diff))
      {
        ...
        if(!m_handler.handle_block_found(b)) {
          ...
          Common::saveStringToFile(m_config_folder_path + "/" + CryptAthlos::parameters::MINER_CONFIG_FILE_NAME, storeToJson(m_config));
        }
      }
