...
namespace CryptAthlos {
  ...
  public:
    ...
    TransactionImpl(const CryptAthlos::Transaction& tx);
  ...
  std::unique_ptr<ITransaction> createTransaction(const CryptAthlos::Transaction& tx) {
    return std::unique_ptr<ITransaction>(new TransactionImpl(tx));
  }

  TransactionImpl::TransactionImpl() {   
    CryptAthlos::KeyPair txKeys(CryptAthlos::generateKeyPair());
  ...
  TransactionImpl::TransactionImpl(const CryptAthlos::Transaction& tx) : transaction(tx) {
    extra.parse(transaction.extra);
  }
  ...
  bool TransactionImpl::findOutputsToAccount(const AccountPublicAddress& addr, const SecretKey& viewSecretKey, std::vector<uint32_t>& out, uint64_t& amount) const {
    return ::CryptAthlos::findOutputsToAccount(transaction, addr, viewSecretKey, out, amount);
  }
