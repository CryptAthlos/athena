...
#include "CryptAthlosSerialization.h"
...
#include "CryptAthlosFormatUtils.h"
#include "CryptAthlosTools.h"
...
struct BinaryVariantTagGetter: boost::static_visitor<uint8_t> {
  uint8_t operator()(const CryptAthlos::BaseInput) { return  0xff; }
  uint8_t operator()(const CryptAthlos::KeyInput) { return  0x2; }
  uint8_t operator()(const CryptAthlos::MultisignatureInput) { return  0x3; }
  uint8_t operator()(const CryptAthlos::KeyOutput) { return  0x2; }
  uint8_t operator()(const CryptAthlos::MultisignatureOutput) { return  0x3; }
  uint8_t operator()(const CryptAthlos::Transaction) { return  0xcc; }
  uint8_t operator()(const CryptAthlos::Block) { return  0xbb; }
};

struct VariantSerializer : boost::static_visitor<> {
  VariantSerializer(CryptAthlos::ISerializer& serializer, const std::string& name) : s(serializer), name(name) {}

  template <typename T>
  void operator() (T& param) { s(param, name); }

  CryptAthlos::ISerializer& s;
  std::string name;
};

void getVariantValue(CryptAthlos::ISerializer& serializer, uint8_t tag, CryptAthlos::TransactionInput& in) {
  switch(tag) {
  case 0xff: {
    CryptAthlos::BaseInput v;
    serializer(v, "value");
    in = v;
    break;
  }
  case 0x2: {
    CryptAthlos::KeyInput v;
    serializer(v, "value");
    in = v;
    break;
  }
  case 0x3: {
    CryptAthlos::MultisignatureInput v;
    serializer(v, "value");
    in = v;
    break;
  }
...
void getVariantValue(CryptAthlos::ISerializer& serializer, uint8_t tag, CryptAthlos::TransactionOutputTarget& out) {
  switch(tag) {
  case 0x2: {
    CryptAthlos::KeyOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
  case 0x3: {
    CryptAthlos::MultisignatureOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
...
template <typename T>
bool serializePod(T& v, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializer.binary(&v, sizeof(v), name);
}

bool serializeVarintVector(std::vector<uint32_t>& vector, CryptAthlos::ISerializer& serializer, Common::StringView name) {
...
bool serialize(PublicKey& pubKey, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializePod(pubKey, name, serializer);
}

bool serialize(SecretKey& secKey, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializePod(secKey, name, serializer);
}

bool serialize(Hash& h, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializePod(h, name, serializer);
}

bool serialize(KeyImage& keyImage, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializePod(keyImage, name, serializer);
}

bool serialize(chacha8_iv& chacha, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializePod(chacha, name, serializer);
}

bool serialize(Signature& sig, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializePod(sig, name, serializer);
}

bool serialize(EllipticCurveScalar& ecScalar, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializePod(ecScalar, name, serializer);
}

bool serialize(EllipticCurvePoint& ecPoint, Common::StringView name, CryptAthlos::ISerializer& serializer) {
