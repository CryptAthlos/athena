...
bool serialize(google::sparse_hash_map<K, V, Hash>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [&value](size_t size) { value.resize(size); });
}

template<typename K, typename Hash>
bool serialize(google::sparse_hash_set<K, Hash>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
...
bool serialize(std::vector<std::pair<Blockchain::TransactionIndex, uint16_t>>& value, Common::StringView name, CryptAthlos::ISerializer& s) {
  ...
  if (s.type() == CryptAthlos::ISerializer::INPUT) {
...
bool Blockchain::checkTransactionInputs(const CryptAthlos::Transaction& tx, BlockInfo& maxUsedBlock) {
  return checkTransactionInputs(tx, maxUsedBlock.height, maxUsedBlock.id);
}

bool Blockchain::checkTransactionInputs(const CryptAthlos::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) {
...
bool Blockchain::haveSpentKeyImages(const CryptAthlos::Transaction& tx) {
...
bool Blockchain::handleGetObjects(NOTIFY_REQUEST_GET_OBJECTS::request& arg, NOTIFY_RESPONSE_GET_OBJECTS::request& rsp) { //Deprecated. Should be removed with CryptAthlosProtocolHandler.
  std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
