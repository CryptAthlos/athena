...
namespace {
  ...
  static bool print_as_json(const T& obj) {
    std::cout << CryptAthlos::storeToJson(obj) << ENDL;
    return true;
  }
}


DaemonCommandsHandler::DaemonCommandsHandler(CryptAthlos::core& core, CryptAthlos::NodeServer& srv, Logging::LoggerManager& log) :
...
{
  std::stringstream ss;
  ss << CryptAthlos::CRYPTATHLOS_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
  ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::start_mining(const std::vector<std::string> &args) {
  ...
  CryptAthlos::AccountPublicAddress adr;
