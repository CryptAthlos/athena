...
#include "CryptAthlosProtocolHandler.h"
...
#include "CryptAthlosCore/VerificationContext.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const CryptAthlosConnectionContext& context) {
...
CryptAthlosProtocolHandler::CryptAthlosProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
...
size_t CryptAthlosProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void CryptAthlosProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void CryptAthlosProtocolHandler::onConnectionOpened(CryptAthlosConnectionContext& context) {
}

void CryptAthlosProtocolHandler::onConnectionClosed(CryptAthlosConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&ICryptAthlosProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != CryptAthlosConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&ICryptAthlosProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void CryptAthlosProtocolHandler::stop() {
  m_stop = true;
}

bool CryptAthlosProtocolHandler::start_sync(CryptAthlosConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == CryptAthlosConnectionContext::state_synchronizing) {
...
bool CryptAthlosProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void CryptAthlosProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const CryptAthlosConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t CryptAthlosProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool CryptAthlosProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, CryptAthlosConnectionContext& context, bool is_inital) {
  if (context.m_state == CryptAthlosConnectionContext::state_befor_handshake && !is_inital)
    return true;
  ...
  if (context.m_state == CryptAthlosConnectionContext::state_synchronizing) {
    ...
    if (is_inital) {
      ...
      context.m_state = CryptAthlosConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = CryptAthlosConnectionContext::state_normal;
    }
  } else {
    ...
    context.m_state = CryptAthlosConnectionContext::state_sync_required;
  }
...
bool CryptAthlosProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, CryptAthlosConnectionContext& ctx, Handler handler) {
...
int CryptAthlosProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, CryptAthlosConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &CryptAthlosProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &CryptAthlosProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &CryptAthlosProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &CryptAthlosProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &CryptAthlosProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &CryptAthlosProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &CryptAthlosProtocolHandler::handleRequestTxPool)
...
int CryptAthlosProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, CryptAthlosConnectionContext& context) {
  ...
  if (context.m_state != CryptAthlosConnectionContext::state_normal) {
    return 1;
  }

  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    CryptAthlos::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
    ...
    if (tvc.m_verifivation_failed) {
      ...
      context.m_state = CryptAthlosConnectionContext::state_shutdown;
      return 1;
    }
  }
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = CryptAthlosConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int CryptAthlosProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, CryptAthlosConnectionContext& context) {
...
int CryptAthlosProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, CryptAthlosConnectionContext& context) {
...
int CryptAthlosProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, CryptAthlosConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(get_block_hash(b))) {
        context.m_state = CryptAthlosConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == CryptAthlosConnectionContext::state_synchronizing) {
...
int CryptAthlosProtocolHandler::processObjects(CryptAthlosConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
...
bool CryptAthlosProtocolHandler::on_idle() {
  return m_core.on_idle();
}

int CryptAthlosProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, CryptAthlosConnectionContext& context) {
...
bool CryptAthlosProtocolHandler::request_missing_objects(CryptAthlosConnectionContext& context, bool check_having_blocks) {
...
bool CryptAthlosProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&ICryptAthlosProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int CryptAthlosProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, CryptAthlosConnectionContext& context) {
...
int CryptAthlosProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     CryptAthlosConnectionContext& context) {
...
void CryptAthlosProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void CryptAthlosProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_TRANSACTIONS::ID, buf);
}

void CryptAthlosProtocolHandler::requestMissingPoolTransactions(const CryptAthlosConnectionContext& context) {
...
void CryptAthlosProtocolHandler::updateObservedHeight(uint32_t peerHeight, const CryptAthlosConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void CryptAthlosProtocolHandler::recalculateMaxObservedHeight(const CryptAthlosConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const CryptAthlosConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t CryptAthlosProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool CryptAthlosProtocolHandler::addObserver(ICryptAthlosProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool CryptAthlosProtocolHandler::removeObserver(ICryptAthlosProtocolObserver* observer) {
