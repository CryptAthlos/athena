...
class WalletService {
public:
  WalletService(const CryptAthlos::Currency& currency, System::Dispatcher& sys, CryptAthlos::INode& node, CryptAthlos::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger);
...
private:
  ...
  std::vector<CryptAthlos::TransactionsInBlockInfo> getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const;
  std::vector<CryptAthlos::TransactionsInBlockInfo> getTransactions(uint32_t firstBlockIndex, size_t blockCount) const;
  ...
  CryptAthlos::IWallet& wallet;
  CryptAthlos::INode& node;
