...
Crypto::Hash parsePaymentId(const std::string& paymentIdStr) {
  if (!checkPaymentId(paymentIdStr)) {
    throw std::system_error(make_error_code(CryptAthlos::error::WalletServiceErrorCode::WRONG_PAYMENT_ID_FORMAT));
  }
...
bool getPaymentIdFromExtra(const std::string& binaryString, Crypto::Hash& paymentId) {
  return CryptAthlos::getPaymentIdFromTxExtra(Common::asBinaryArray(binaryString), paymentId);
}
...
struct TransactionsInBlockInfoFilter {
  ...
  bool checkTransaction(const CryptAthlos::WalletTransactionWithTransfers& transaction) const {
    if (havePaymentId) {
    ...
    for (const CryptAthlos::WalletTransfer& transfer: transaction.transfers) {
...
void addPaymentIdToExtra(const std::string& paymentId, std::string& extra) {
  ...
  if (!CryptAthlos::createTxExtraWithPaymentId(paymentId, extraVector)) {
...
Crypto::Hash parseHash(const std::string& hashString, Logging::LoggerRef logger) {
  ...
  if (!Common::podFromHex(hashString, hash)) {
    ...
    throw std::system_error(make_error_code(CryptAthlos::error::WalletServiceErrorCode::WRONG_HASH_FORMAT));
  }

  return hash;
}

std::vector<CryptAthlos::TransactionsInBlockInfo> filterTransactions(
  const std::vector<CryptAthlos::TransactionsInBlockInfo>& blocks,
  const TransactionsInBlockInfoFilter& filter) {

  std::vector<CryptAthlos::TransactionsInBlockInfo> result;

  for (const auto& block: blocks) {
    CryptAthlos::TransactionsInBlockInfo item;
    item.blockHash = block.blockHash;

    for (const auto& transaction: block.transactions) {
      if (transaction.transaction.state != CryptAthlos::WalletTransactionState::DELETED && filter.checkTransaction(transaction)) {
...
PaymentService::TransactionRpcInfo convertTransactionWithTransfersToTransactionRpcInfo(
  const CryptAthlos::WalletTransactionWithTransfers& transactionWithTransfers) {
  ...
  for (const CryptAthlos::WalletTransfer& transfer: transactionWithTransfers.transfers) {
...
std::vector<PaymentService::TransactionsInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionsInBlockRpcInfo(
  ...
  for (const auto& block: blocks) {
    ...
    for (const CryptAthlos::WalletTransactionWithTransfers& transactionWithTransfers: block.transactions) {
...
std::vector<PaymentService::TransactionHashesInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(
  ...
  for (const CryptAthlos::TransactionsInBlockInfo& block: blocks) {
    PaymentService::TransactionHashesInBlockRpcInfo item;
    item.blockHash = Common::podToHex(block.blockHash);

    for (const CryptAthlos::WalletTransactionWithTransfers& transaction: block.transactions) {
...
void validateAddresses(const std::vector<std::string>& addresses, const CryptAthlos::Currency& currency, Logging::LoggerRef logger) {
...
std::vector<CryptAthlos::WalletOrder> convertWalletRpcOrdersToWalletOrders(const std::vector<PaymentService::WalletRpcOrder>& orders) {
  std::vector<CryptAthlos::WalletOrder> result;
  result.reserve(orders.size());

  for (const auto& order: orders) {
    result.emplace_back(CryptAthlos::WalletOrder {order.address, order.amount});
  }
...
void saveWallet(CryptAthlos::IWallet& wallet, std::fstream& walletFile, bool saveDetailed = true, bool saveCache = true) {
  wallet.save(walletFile, saveDetailed, saveCache);
  walletFile.flush();
}

void secureSaveWallet(CryptAthlos::IWallet& wallet, const std::string& path, bool saveDetailed = true, bool saveCache = true) {
...
void generateNewWallet(const CryptAthlos::Currency &currency, const WalletConfiguration &conf, Logging::ILogger& logger, System::Dispatcher& dispatcher) {
  Logging::LoggerRef log(logger, "generateNewWallet");

  CryptAthlos::INode* nodeStub = NodeFactory::createNodeStub();
  std::unique_ptr<CryptAthlos::INode> nodeGuard(nodeStub);

  CryptAthlos::IWallet* wallet = WalletFactory::createWallet(currency, *nodeStub, dispatcher);
  std::unique_ptr<CryptAthlos::IWallet> walletGuard(wallet);
...
void importLegacyKeys(const std::string &legacyKeysFile, const WalletConfiguration &conf) {
  ...
  CryptAthlos::importLegacyKeys(legacyKeysFile, conf.walletPassword, archive);
...
WalletService::WalletService(const CryptAthlos::Currency& currency, System::Dispatcher& sys, CryptAthlos::INode& node,
  CryptAthlos::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger) :
    currency(currency),
    wallet(wallet),
    node(node),
    config(conf),
...
std::error_code WalletService::resetWallet() {
  try {
    ...
    if (!inited) {
      ...
      return make_error_code(CryptAthlos::error::NOT_INITIALIZED);
    }
    ...
    logger(Logging::WARNING) << "Error while reseting wallet: " << x.what();
    return make_error_code(CryptAthlos::error::INTERNAL_WALLET_ERROR);
  }
...
std::error_code WalletService::replaceWithNewWallet(const std::string& viewSecretKeyText) {
  try {
    ...
    if (!Common::podFromHex(viewSecretKeyText, viewSecretKey)) {
      ...
      return make_error_code(CryptAthlos::error::WalletServiceErrorCode::WRONG_KEY_FORMAT);
    }
...
std::error_code WalletService::getSpendkeys(const std::string& address, std::string& publicSpendKeyText, std::string& secretSpendKeyText) {
  try {
    ...
    CryptAthlos::KeyPair key = wallet.getAddressSpendKey(address);
...
std::error_code WalletService::getViewKey(std::string& viewSecretKey) {
  try {
    System::EventLock lk(readyEvent);
    CryptAthlos::KeyPair viewKey = wallet.getViewKey();
...
std::error_code WalletService::getTransaction(const std::string& transactionHash, TransactionRpcInfo& transaction) {
  try {
    ...
    CryptAthlos::WalletTransactionWithTransfers transactionWithTransfers = wallet.getTransaction(hash);

    if (transactionWithTransfers.transaction.state == CryptAthlos::WalletTransactionState::DELETED) {
      logger(Logging::WARNING) << "Transaction " << transactionHash << " is deleted";
      return make_error_code(CryptAthlos::error::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::sendTransaction(const SendTransaction::Request& request, std::string& transactionHash) {
  try {
    ...
    CryptAthlos::TransactionParameters sendParams;
...
std::error_code WalletService::deleteDelayedTransaction(const std::string& transactionHash) {
  try {
    ...
    if (idIt == transactionIdIndex.end()) {
      return make_error_code(CryptAthlos::error::WalletServiceErrorCode::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::getUnconfirmedTransactionHashes(const std::vector<std::string>& addresses, std::vector<std::string>& transactionHashes) {
  try {
    ...
    std::vector<CryptAthlos::WalletTransactionWithTransfers> transactions = wallet.getUnconfirmedTransactions();
...
void WalletService::refresh() {
  try {
    ...
    for (;;) {
      ...
      if (event.type == CryptAthlos::TRANSACTION_CREATED) {
...
std::vector<CryptAthlos::TransactionsInBlockInfo> WalletService::getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const {
  std::vector<CryptAthlos::TransactionsInBlockInfo> result = wallet.getTransactions(blockHash, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(CryptAthlos::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }

  return result;
}

std::vector<CryptAthlos::TransactionsInBlockInfo> WalletService::getTransactions(uint32_t firstBlockIndex, size_t blockCount) const {
  std::vector<CryptAthlos::TransactionsInBlockInfo> result = wallet.getTransactions(firstBlockIndex, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(CryptAthlos::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(const Crypto::Hash& blockHash, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<CryptAthlos::TransactionsInBlockInfo> allTransactions = getTransactions(blockHash, blockCount);
  std::vector<CryptAthlos::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(uint32_t firstBlockIndex, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<CryptAthlos::TransactionsInBlockInfo> allTransactions = getTransactions(firstBlockIndex, blockCount);
  std::vector<CryptAthlos::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
  return convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(filteredTransactions);
}
