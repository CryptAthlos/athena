...
serializeAsBinary(std::vector<T>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  std::string blob;
...
template<typename T>
typename std::enable_if<std::is_pod<T>::value>::type
serializeAsBinary(std::list<T>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  std::string blob;
  if (serializer.type() == ISerializer::INPUT) {
    serializer.binary(blob, name);
...
bool serializeContainer(Cont& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
...
template <typename E>
bool serializeEnumClass(E& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  static_assert(std::is_enum<E>::value, "E must be an enum class");

  typedef typename std::underlying_type<E>::type EType;

  if (serializer.type() == CryptAthlos::ISerializer::INPUT) {
    EType numericValue;
...
template<typename T>
bool serialize(std::vector<T>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeContainer(value, name, serializer);
}

template<typename T>
bool serialize(std::list<T>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeContainer(value, name, serializer);
}

template<typename MapT, typename ReserveOp>
bool serializeMap(MapT& value, Common::StringView name, CryptAthlos::ISerializer& serializer, ReserveOp reserve) {
  size_t size = value.size();
...
bool serializeSet(SetT& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  size_t size = value.size();
...
bool serialize(std::unordered_set<K, Hash>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeSet(value, name, serializer);
}

template<typename K, typename Cmp>
bool serialize(std::set<K, Cmp>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeSet(value, name, serializer);
}

template<typename K, typename V, typename Hash>
bool serialize(std::unordered_map<K, V, Hash>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [&value](size_t size) { value.reserve(size); });
}

template<typename K, typename V, typename Hash>
bool serialize(std::unordered_multimap<K, V, Hash>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [&value](size_t size) { value.reserve(size); });
}

template<typename K, typename V, typename Hash>
bool serialize(std::map<K, V, Hash>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [](size_t size) {});
}

template<typename K, typename V, typename Hash>
bool serialize(std::multimap<K, V, Hash>& value, Common::StringView name, CryptAthlos::ISerializer& serializer) {
  return serializeMap(value, name, serializer, [](size_t size) {});
}

template<size_t size>
bool serialize(std::array<uint8_t, size>& value, Common::StringView name, CryptAthlos::ISerializer& s) {
