...
#include "CryptAthlosProtocol/CryptAthlosProtocolHandler.h"
...
struct TransferCommand {
  ...
  std::vector<CryptAthlos::WalletLegacyTransfer> dsts;
  std::vector<uint8_t> extra;
  uint64_t fee;

  TransferCommand(const CryptAthlos::Currency& currency) :
      ...
      while (!ar.eof()) {
        ...
        if (arg.size() && arg[0] == '-') {
          ...
          if (!m_currency.parseAccountAddressString(arg, de.addr)) {
            ...
            if (CryptAthlos::parsePaymentId(arg, paymentId)) {
...
std::string tryToOpenWalletOrLoadKeysOrThrow(LoggerRef& logger, std::unique_ptr<IWalletLegacy>& wallet, const std::string& walletFile, const std::string& password) {
  ...
  if (walletExists) {
    ...
    if (initError) { //bad password, or legacy format
      if (keysExists) {
        std::stringstream ss;
        CryptAthlos::importLegacyKeys(keys_file, password, ss);
        ...
        try {
          CryptAthlos::WalletHelper::storeWallet(*wallet, walletFileName);
...
simple_wallet::simple_wallet(System::Dispatcher& dispatcher, const CryptAthlos::Currency& currency, Logging::LoggerManager& log) :
...
bool simple_wallet::new_wallet(const std::string &wallet_file, const std::string& password) {
  ...
  try {
    ...
    try {
      CryptAthlos::WalletHelper::storeWallet(*m_wallet, m_wallet_file);
    } catch (std::exception& e) {
...
//----------------------------------------------------------------------------------------------------
void simple_wallet::externalTransactionCreated(CryptAthlos::TransactionId transactionId)  {
...
bool simple_wallet::show_payments(const std::vector<std::string> &args) {
  ...
  for (const std::string& arg: args) {
    ...
    if (CryptAthlos::parsePaymentId(arg, expectedPaymentId)) {
      size_t transactionsCount = m_wallet->getTransactionCount();
      for (size_t trantransactionNumber = 0; trantransactionNumber < transactionsCount; ++trantransactionNumber) {
        WalletLegacyTransaction txInfo;
        m_wallet->getTransaction(trantransactionNumber, txInfo);
        ...
        Crypto::Hash paymentId;
        if (CryptAthlos::getPaymentIdFromTxExtra(extraVec, paymentId) && paymentId == expectedPaymentId) {
          payments_found = true;
...
bool simple_wallet::transfer(const std::vector<std::string> &args) {
  try {
    ...
    if (!cmd.parseArguments(logger, args))
      return false;
    CryptAthlos::WalletHelper::SendCompleteResultObserver sent;
    ...
    CryptAthlos::TransactionId tx = m_wallet->sendTransaction(cmd.dsts, cmd.fee, extraString, cmd.fake_outs_count, 0);
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    if (command_line::get_arg(vm, command_line::arg_help)) {
      CryptAthlos::Currency tmp_currency = CryptAthlos::CurrencyBuilder(logManager).currency();
      CryptAthlos::simple_wallet tmp_wallet(dispatcher, tmp_currency, logManager);

      std::cout << CRYPTATHLOS_NAME << " wallet v" << PROJECT_VERSION_LONG << std::endl;
      ...
      std::cout << CRYPTATHLOS_NAME << " wallet v" << PROJECT_VERSION_LONG;
      return false;
    }
  ...
  logger(INFO, BRIGHT_WHITE) << CRYPTATHLOS_NAME << " wallet v" << PROJECT_VERSION_LONG;

  CryptAthlos::Currency currency = CryptAthlos::CurrencyBuilder(logManager).
  ...
  if (command_line::has_arg(vm, Tools::wallet_rpc_server::arg_rpc_bind_port)) {
    ...
    CryptAthlos::simple_wallet wal(dispatcher, currency, logManager);
