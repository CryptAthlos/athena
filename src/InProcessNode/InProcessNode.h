...
class InProcessNode : public INode, public CryptAthlos::ICryptAthlosProtocolObserver, public CryptAthlos::ICoreObserver {
public:
  InProcessNode(CryptAthlos::ICore& core, CryptAthlos::ICryptAthlosProtocolQuery& protocol);
...
private:
  ...
  void getNewBlocksAsync(std::vector<Crypto::Hash>& knownBlockIds, std::vector<CryptAthlos::block_complete_entry>& newBlocks, uint32_t& startHeight, const Callback& callback);
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<CryptAthlos::block_complete_entry>& newBlocks, uint32_t& startHeight);
  ...
  std::error_code doGetRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount,
      std::vector<CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result);

  void relayTransactionAsync(const CryptAthlos::Transaction& transaction, const Callback& callback);
  std::error_code doRelayTransaction(const CryptAthlos::Transaction& transaction);
  ...
  State state;
  CryptAthlos::ICore& core;
  CryptAthlos::ICryptAthlosProtocolQuery& protocol;
