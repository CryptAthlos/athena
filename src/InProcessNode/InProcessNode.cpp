...
#include "CryptAthlosProtocol/CryptAthlosProtocolHandlerCommon.h"
...
InProcessNode::InProcessNode(CryptAthlos::ICore& core, CryptAthlos::ICryptAthlosProtocolQuery& protocol) :
...
void InProcessNode::init(const Callback& callback) {
  ...
  if (state != NOT_INITIALIZED) {
    ec = make_error_code(CryptAthlos::error::ALREADY_INITIALIZED);
  } else {
...
void InProcessNode::getNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<CryptAthlos::block_complete_entry>& newBlocks,
  uint32_t& startHeight, const Callback& callback)
{
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
    callback(make_error_code(CryptAthlos::error::NOT_INITIALIZED));
    return;
  }
...
void InProcessNode::getNewBlocksAsync(std::vector<Crypto::Hash>& knownBlockIds, std::vector<CryptAthlos::block_complete_entry>& newBlocks,
  uint32_t& startHeight, const Callback& callback)
{
...
std::error_code InProcessNode::doGetNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<CryptAthlos::block_complete_entry>& newBlocks, uint32_t& startHeight) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
  ...
  try {
    ...
    if (knownBlockIds.empty()) {
      return make_error_code(CryptAthlos::error::REQUEST_ERROR);
    }
    ...
    std::vector<Crypto::Hash> supplement = core.findBlockchainSupplement(knownBlockIds, CryptAthlos::COMMAND_RPC_GET_BLOCKS_FAST_MAX_COUNT, totalBlockCount, startHeight);
    ...
    for (const auto& blockId : supplement) {
      ...
      CryptAthlos::block_complete_entry be;
    ...
    return make_error_code(CryptAthlos::error::INTERNAL_NODE_ERROR);
  }
...
void InProcessNode::getRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount,
    std::vector<CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result, const Callback& callback)
{
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
...
//it's always protected with mutex
std::error_code InProcessNode::doGetRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount, std::vector<CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
      return make_error_code(CryptAthlos::error::NOT_INITIALIZED);
  ...
  try {
    CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::response res;
    CryptAthlos::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::request req;
    req.amounts = amounts;
...
void InProcessNode::relayTransaction(const CryptAthlos::Transaction& transaction, const Callback& callback)
{
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
...
void InProcessNode::relayTransactionAsync(const CryptAthlos::Transaction& transaction, const Callback& callback) {
  std::error_code ec = doRelayTransaction(transaction);
  callback(ec);
}

//it's always protected with mutex
std::error_code InProcessNode::doRelayTransaction(const CryptAthlos::Transaction& transaction) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
      return make_error_code(CryptAthlos::error::NOT_INITIALIZED);
  ...
  try {
    CryptAthlos::BinaryArray transactionBinaryArray = toBinaryArray(transaction);
    CryptAthlos::tx_verification_context tvc = boost::value_initialized<CryptAthlos::tx_verification_context>();
    ...
    CryptAthlos::NOTIFY_NEW_TRANSACTIONS::request r;
...
uint64_t InProcessNode::getLastLocalBlockTimestamp() const {
  ...
  if (!core.getBlockByHash(hash, block)) {
    throw std::system_error(make_error_code(CryptAthlos::error::INTERNAL_NODE_ERROR));
  }
...
std::error_code InProcessNode::doQueryBlocksLite(std::vector<Crypto::Hash>&& knownBlockIds, uint64_t timestamp, std::vector<BlockShortEntry>& newBlocks, uint32_t& startHeight) {
  ...
  std::vector<CryptAthlos::BlockShortInfo> entries;
